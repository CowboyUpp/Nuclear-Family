...
;(function (window, document) {
  ...
  function isLocalRegistryOnlyNotice() {
    return '<div style="font-size:11px; color:#f59e0b; line-height:1.5; padding:9px 11px; border:1px solid #f59e0b; border-radius:7px; background:rgba(245,158,11,0.08); margin-bottom:12px;">' +
      '<strong>Security note:</strong> This registry is local planning data only. It does not grant backend permissions. Real upload/admin permissions are controlled only by backend-issued tokens and server-side validation.' +
      '</div>';
  }
  ...
  function loadControlSectionState() {
    try {
      return JSON.parse(localStorage.getItem(CONFIG_CC_SECTION_KEY) || '{}') || {};
    } catch (err) {
      return {};
    }
  }
  ...
  function saveControlSectionState(state) {
    localStorage.setItem(CONFIG_CC_SECTION_KEY, JSON.stringify(state || {}));
  }

  function isControlSectionOpen(sectionId, defaultOpen) {
    var state = loadControlSectionState();
    if (Object.prototype.hasOwnProperty.call(state, sectionId)) return !!state[sectionId];
    return !!defaultOpen;
  }
  ...
  function renderAdminStewardRegistryBody() {
    ...
    if (!total) {
      rows = '<div style="font-size:12px; color:var(--nbf-mut); padding:10px 0;">No stewards registered locally yet.</div>';
    } else {
    ...
    return '' +
      ...
      renderStatModule('Registered', total) +
      renderStatModule('Authority', 'Backend Only') +
      renderStatModule('Local Registry', 'Display Only') +
      '</div>' +
      '<div style="background:var(--nbf-main); border:1px solid var(--nbf-bld); border-radius:7px; padding:10px 12px; margin-bottom:12px;">' +
      rows +
      '</div>' +
      ...
      '  </select>' +
      '  <button id="nbf-steward-add-btn" style="padding:7px 12px; background:#16a34a; color:#fff; border:none; border-radius:6px; font-size:12px; font-weight:600; cursor:pointer;">Add Local Entry</button>' +
      '</div>' +
      '<div id="nbf-steward-add-msg" style="display:none; font-size:11px; margin-top:8px;"></div>' +
      '<div style="font-size:11px; color:var(--nbf-mut); line-height:1.5; margin-top:12px;">Backend-backed steward identities, token hashes, last-seen status, assigned competitions, and audit history are planned later. Until then, this list is only a local admin notebook.</div>';
  }
  ...
  function renderStewardPanelContent() {
    ...
    var adminHtml = '' +
      ...
      '    <span style="font-size:11px; color:var(--nbf-mut);">Garage diagnostics and future management tools — admin token is never stored here</span>' +
      '  </div>' +
      renderControlAccordion('admin-system-health', 'System Health', BACKEND_STATUS.backend || 'Not checked', renderAdminSystemHealthBody(), true) +
      renderControlAccordion('admin-steward-registry', 'Steward Registry', STEWARD_REGISTRY.length + ' local', renderAdminStewardRegistryBody(), true) +
    ...
    var stewardHtml = '' +
      ...
      renderControlAccordion('steward-auth', 'Steward Access', STATE_STEWARD_TOKEN ? 'configured' : 'missing', stewardTokenBody, true) +
      renderControlAccordion('steward-upload-status', 'Upload Status', LAST_INGESTION_STATUS.message || 'No upload yet', renderStewardUploadStatusBody(), true) +
    ...
    for (var t = 0; t < toggles.length; t++) {
      toggles[t].addEventListener('click', function() {
        ...
        var state = loadControlSectionState();
        state[sectionId] = !isControlSectionOpen(sectionId, false);
        saveControlSectionState(state);
        renderStewardPanelContent();
      });
    }

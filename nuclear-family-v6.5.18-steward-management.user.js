...
;(function (window, document) {
  ...
  function renderModalInterface() {
    ...
    viewHtml += '    </div>';
    viewHtml += '    <button id="nbf-tab-stewards" class="nbf-tab-btn" style="margin-left:auto;">Control Center</button>';
    viewHtml += '  </div>';
  ...
  function renderRoleBadge(role) {
    ...
    return '<span style="display:inline-block; padding:3px 8px; border-radius:999px; background:' + bg + '; color:' + color + '; border:1px solid ' + border + '; font-size:10px; font-weight:800; letter-spacing:.02em; white-space:nowrap;">' + escapeHTML(label) + '</span>';
  }

  function renderStatusLight(active, lastSeen) {
    if (Number(active) !== 1) {
      return '<span style="display:inline-flex; align-items:center; gap:5px; color:#f59e0b; font-weight:700;"><span style="width:10px; height:10px; border-radius:50%; background:#f59e0b; box-shadow:0 0 8px rgba(245,158,11,0.55);"></span>Disabled</span>';
    }

    var detail = lastSeen ? 'Seen' : 'Active';
    return '<span style="display:inline-flex; align-items:center; gap:5px; color:#16a34a; font-weight:700;"><span style="width:10px; height:10px; border-radius:50%; background:#16a34a; box-shadow:0 0 8px rgba(22,163,74,0.55);"></span>' + detail + '</span>';
  }
  ...
  function renderAdminUnlockBox() {
    if (isAdminUnlocked()) {
      return '' +
        ...
        '    <span style="color:var(--nbf-mut);">Backend authenticated for this browser tab/session. Token is not saved to localStorage.</span>' +
        '  </div>' +
        '  <button id="nbf-admin-lock-btn" style="padding:7px 12px; background:var(--nbf-field-bg); color:#dc2626; border:1px solid #fca5a5; border-radius:6px; font-size:12px; cursor:pointer;">Lock</button>' +
        '</div>';
    }

    return '' +
      '<div style="font-size:11px; color:var(--nbf-txt); line-height:1.5; padding:10px 12px; border:1px solid #60a5fa; border-radius:7px; background:rgba(96,165,250,0.10); margin-bottom:12px;">' +
      '  <strong>🟡 Admin Mode Locked</strong><br>' +
      '  <span style="color:var(--nbf-mut);">Paste ADMIN_TOKEN to unlock backend registry. Stored only for this browser tab/session.</span>' +
      '  <div style="display:flex; gap:8px; margin-top:10px; flex-wrap:wrap;">' +
      '    <input id="nbf-admin-token-input" type="password" placeholder="Paste ADMIN_TOKEN to unlock backend registry" autocomplete="new-password" style="flex:1; min-width:260px; padding:7px 9px; border:1px solid var(--nbf-btn-border); border-radius:6px; font-size:12px; background:var(--nbf-field-bg); color:var(--nbf-txt);" />' +
      '    <button id="nbf-admin-unlock-btn" style="padding:7px 12px; background:#6366f1; color:#fff; border:none; border-radius:6px; font-size:12px; font-weight:600; cursor:pointer;">Unlock</button>' +
      '  </div>' +
      '</div>';
  }
  ...
  function renderBackendStewardRows() {
    ...
    var rows = '<div style="display:grid; grid-template-columns:54px 76px 1fr 128px 94px 112px 130px; gap:8px; padding:7px 0; border-bottom:1px solid var(--nbf-bld); font-size:11px; font-weight:700; color:var(--nbf-txt); min-width:980px;">' +
      '<div>ID</div><div>Torn ID</div><div>Name</div><div>Role</div><div>Status</div><div>Created</div><div>Actions</div></div>';

    stewards.forEach(function(steward) {
      var isActive = Number(steward.active) === 1;
      rows += '<div style="display:grid; grid-template-columns:54px 76px 1fr 128px 94px 112px 130px; gap:8px; align-items:center; padding:8px 0; border-bottom:1px solid var(--nbf-bld); font-size:11px; color:var(--nbf-mut); min-width:980px;">' +
        '<div style="font-family:monospace;">' + escapeHTML(steward.id || '—') + '</div>' +
        '<div style="font-family:monospace;">' + escapeHTML(steward.torn_id || '—') + '</div>' +
        '<div style="color:var(--nbf-txt); font-weight:700;">' + escapeHTML(steward.display_name || 'Unknown') + '</div>' +
        '<div>' + renderRoleBadge(steward.role || 'event') + '</div>' +
        '<div>' + renderStatusLight(steward.active, steward.last_seen_at) + '</div>' +
        '<div style="font-family:monospace; font-size:10px;">' + escapeHTML((steward.created_at || '—').slice(0, 10)) + '</div>' +
        '<div style="display:flex; gap:5px; flex-wrap:wrap;">' +
        '  <button class="nbf-backend-steward-edit-btn" data-id="' + escapeHTML(steward.id) + '" style="padding:4px 7px; background:var(--nbf-field-bg); color:var(--nbf-txt); border:1px solid var(--nbf-btn-border); border-radius:5px; font-size:10px; cursor:pointer;">Edit</button>' +
        '  <button class="nbf-backend-steward-toggle-btn" data-id="' + escapeHTML(steward.id) + '" data-active="' + (isActive ? '1' : '0') + '" style="padding:4px 7px; background:var(--nbf-field-bg); color:' + (isActive ? '#f59e0b' : '#16a34a') + '; border:1px solid var(--nbf-btn-border); border-radius:5px; font-size:10px; cursor:pointer;">' + (isActive ? 'Disable' : 'Enable') + '</button>' +
        '  <button class="nbf-backend-steward-token-btn" data-id="' + escapeHTML(steward.id) + '" style="padding:4px 7px; background:var(--nbf-field-bg); color:#6366f1; border:1px solid var(--nbf-btn-border); border-radius:5px; font-size:10px; cursor:pointer;">Token</button>' +
        '</div>' +
        '</div>';
    });
  ...
  function renderRoleOption(value, current) {
    return '<option value="' + value + '"' + (String(current || '') === value ? ' selected' : '') + '>' + value + '</option>';
  }
  ...
  function renderBackendStewardRegistryCard() {
    ...
    return '' +
      ...
      '<div style="background:var(--nbf-main); border:1px solid var(--nbf-bld); border-radius:7px; padding:10px 12px; margin-bottom:12px; overflow-x:auto;">' +
  ...
  function renderStewardUploadStatusBody() {
    return renderIngestionStatusMarkup('✅ ' + CF_INGEST_ENDPOINT);
  }
  ...
  function renderStewardPanelContent() {
    ...
    if (adminUnlockBtn) adminUnlockBtn.addEventListener('click', function() {
      var input = document.getElementById('nbf-admin-token-input');
      var token = input ? input.value.trim() : '';
      if (!token) {
        showScraperToast('Admin token required.', 'error', 2500);
        return;
      }
      STATE_ADMIN_TOKEN_SESSION = token;
      sessionStorage.setItem(CONFIG_ADMIN_SESSION_KEY, token);
      fetchBackendStewards();
    });
  ...
  function transmitRacePayload(payload) {
    // Torn compliance guard: never transmit data from an unfocused/hidden window.
    if (!canScrapeRace('payload transmission')) return;
    ...
    setGateStatus('backend', 'uploading');
    ...
    .then(function(res) {
      return res.text().then(function(txt) {
        var parsed = null;
        try { parsed = txt ? JSON.parse(txt) : null; } catch (err) {}

        if (res.ok) {
          markRaceSubmitted(payload.race_id, 'submitted');
          setGateStatus('backend', 'online: accepted');
          setLastIngestionStatus('success', 'Uploaded successfully.', payload.race_id, payload.results.length);
          showScraperToast('✅ Race #' + payload.race_id + ' ingested successfully (' + payload.results.length + ' drivers).', 'success', 5000);
          console.log('[NB Racing] Ingestion success for race ' + payload.race_id, parsed || txt);
          return;
        }

        if (res.status === 409) {
          markRaceSubmitted(payload.race_id, 'duplicate');
          setGateStatus('backend', 'duplicate blocked');
          setLastIngestionStatus('duplicate', 'Already existed on backend.', payload.race_id, payload.results.length);
          showScraperToast('🟡 Race #' + payload.race_id + ' already exists on backend. Marked as submitted.', 'info', 5000);
          console.warn('[NB Racing] Duplicate race blocked by backend:', parsed || txt);
          return;
        }

        setGateStatus('backend', 'error: HTTP ' + res.status);
        setLastIngestionStatus('error', 'Failed — HTTP ' + res.status + '.', payload.race_id, payload.results.length);
        showScraperToast('❌ Ingestion failed: HTTP ' + res.status + '. Check console.', 'error', 6000);
        console.error('[NB Racing] Ingestion HTTP error:', res.status, parsed || txt);
      });
    })
    .catch(function(err) {
      setGateStatus('backend', 'error: network');
      setLastIngestionStatus('error', 'Network error — payload not delivered.', payload.race_id, payload.results.length);

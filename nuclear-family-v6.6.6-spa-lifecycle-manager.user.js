...
;(function (window, document) {
  ...
  function mountFloatingInterface() {
    ...
    function getLauncherStatus() {
      ...
      try {
        if (STATE_SYNC_ACTIVE) return { label: 'Syncing', color: '#d97706', cls: 'syncing' };
      } catch (err2) {}

      try {
        if (LAST_BACKEND_HEALTH && LAST_BACKEND_HEALTH.ok === false) {
          return { label: 'Offline', color: '#dc2626', cls: 'offline' };
        }
      } catch (err3) {}
      ...
      return { label: 'Connected', color: '#16a34a', cls: 'connected' };
    }
...
document.getElementById('nbf-cfg-type').addEventListener('change', function() {
  ...
  function createCompetitionFromTemplate() {
    ...
    if (!name) {
      showScraperToast('Competition name required.', 'error', 2500);
      return;
    }
    ...
    competitionRequest(CF_COMPETITIONS_ENDPOINT, {
      ...
      showScraperToast('Competition created.', 'success', 2200);
      fetchCompetitionSetupData();
    }).catch(function(err) {
      showScraperToast('Competition create failed: ' + (err && err.message ? err.message : String(err)), 'error', 3500);
    });
  }
  ...
  function renderCompetitionSetupBody() {
    ...
    var html = '<div id="nbf-competition-setup-panel">' +
      ...
      renderStatModule('Competitions', competitions.length) +
      ...
      '<h4 style="margin:0 0 8px 0; color:var(--nbf-txt); font-size:12px;">Create Competition</h4>' +
      '<input id="nbf-comp-name" placeholder="Competition name, e.g. Nuclear Family JLT" style="width:100%; box-sizing:border-box; margin-bottom:6px; padding:7px 9px; border:1px solid var(--nbf-btn-border); border-radius:6px; background:var(--nbf-field-bg); color:var(--nbf-txt); font-size:12px;">' +
      ...
      '<button id="nbf-comp-create-btn" style="padding:7px 12px; background:#16a34a; color:#fff; border:none; border-radius:6px; font-size:12px; font-weight:600; cursor:pointer;">Create Competition</button>' +
    ...
    html += '<h4 style="margin:8px 0; font-size:12px; color:var(--nbf-txt);">Competitions</h4>';
  ...
  function renderStewardPanelContent() {
    ...
    var adminHtml = '' +
      ...
      renderControlAccordion('admin-competition-manager', 'Competition Manager', (COMPETITION_SETUP_STATE.competitions.length || 0) + ' competition(s)', renderCompetitionSetupBody, false) +
  ...
  function nbfArticleGettingStarted() {
    return '<h2 style="margin:0 0 8px;color:var(--nbf-txt);">Getting Started</h2>'+
      ...
      nbfDocStep(5,'Create a competition template','In Competition Manager, choose JLT, URT or Custom. Templates store rules for grouping, scoring and advancement.','Templates are rule blueprints.')+
  ...
  function nbfArticleTemplates() {
    return '<h2 style="margin:0 0 8px;color:var(--nbf-txt);">Competition Templates</h2><p style="font-size:13px;line-height:1.6;color:var(--nbf-mut);">A template is a rule blueprint. It defines registration, grouping, scheduling, scoring and advancement.</p>'+
  ...
  function nbfArticleRoadmap() {
    return '<h2 style="margin:0 0 8px;color:var(--nbf-txt);">Roadmap</h2>'+
      nbfDocProgress('Backend',100)+nbfDocProgress('Security',90)+nbfDocProgress('Steward System',90)+nbfDocProgress('Upload Pipeline',95)+nbfDocProgress('Control Center',85)+nbfDocProgress('Competition Templates',70)+nbfDocProgress('Participant Import',65)+nbfDocProgress('Championship Standings',50)+nbfDocProgress('Archive',20)+nbfDocProgress('Help Central',80)+
      nbfDocBox('info','Development direction','The next layer is turning template configuration into automatic league generation, track-specific qualifier rankings, race-to-event helpers and organizer-facing standings/export tools.');
  }
  ...
  function renderHelpCentralDocs() {
    var docs = [
      ...
      ['templates','Competition Templates',nbfArticleTemplates],
...
function renderHelpPanelContent() {
  ...
  function nbfStartLifecycleManager() {
    ...
    NBF_LIFECYCLE.observer.observe(document.body || document.documentElement, {
      childList: true,
      subtree: true
    });

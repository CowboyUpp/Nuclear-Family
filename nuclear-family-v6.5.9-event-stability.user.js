...
;(function (window, document) {
  ...
  function renderApiKeyControlCard() {
    ...
    var isOpen = localStorage.getItem(CONFIG_API_ACCORDION_KEY) === 'open';
    var bodyDisplay = isOpen ? 'block' : 'none';
    var arrow = isOpen ? '▲' : '▼';

    return '' +
      '<div id="nbf-api-accordion" style="background:var(--nbf-alt2); border:1px solid var(--nbf-bld); border-radius:8px; margin-bottom:14px; overflow:hidden;">' +
      '  <div id="nbf-api-accordion-toggle" style="padding:12px 14px; cursor:pointer; display:flex; align-items:center; justify-content:space-between; gap:12px; background:var(--nbf-alt);">' +
      '    <strong style="font-size:13px; color:var(--nbf-txt);">' + arrow + ' Torn Account</strong>' +
      '    <span style="font-size:11px; color:' + (keyLength ? '#16a34a' : '#f59e0b') + '; font-weight:600;">' + (keyLength ? 'API key configured' : 'API key missing') + '</span>' +
      '  </div>' +
      '  <div id="nbf-api-accordion-body" style="display:' + bodyDisplay + '; padding:16px;">' +
  ...
  function renderStewardPanelContent() {
    ...
    panelHtml += '  <p style="margin:4px 0 0 0; font-size:12px; color:var(--nbf-mut);">Admin and Steward operations live here: backend checks, tokens, API access, upload status, diagnostics, and future competition tools.</p>';
    panelHtml += '</div>';

    panelHtml += renderApiKeyControlCard();

    panelHtml += renderBackendStatusMarkup();
    ...
    panelHtml += renderIngestionStatusMarkup(endpointStatus);
    panelHtml += renderControlCenterMarkup();

    panelHtml += '</div>'; // end outer wrapper
    targetContainer.innerHTML = panelHtml;

    var apiAccordionToggle = document.getElementById('nbf-api-accordion-toggle');
    if (apiAccordionToggle) nbfBindOnce(apiAccordionToggle, 'click', function() {
      var body = document.getElementById('nbf-api-accordion-body');
      if (!body) return;
      var willOpen = body.style.display === 'none';
      localStorage.setItem(CONFIG_API_ACCORDION_KEY, willOpen ? 'open' : 'closed');
      renderStewardPanelContent();
    }, 'api_accordion_toggle');

    var controlApiBtn = document.getElementById('nbf-control-api-open');
    if (controlApiBtn) nbfBindOnce(controlApiBtn, 'click', function() {
    ...
    if (backendCheckBtn) nbfBindOnce(backendCheckBtn, 'click', function() { refreshBackendStatus(true); }, 'backend_check_panel');
    var ingestClearBtn = document.getElementById('nbf-ingest-clear-local');
    if (ingestClearBtn) ingestClearBtn.addEventListener('click', function() {
      clearSubmittedRaceMap();
      clearLastIngestionStatus();
      addActivityLog('Local ingestion history cleared.', 'warning');
      renderStewardPanelContent();
    });
    var controlToggle = document.getElementById('nbf-control-toggle');
    if (controlToggle) controlToggle.addEventListener('click', function() {
      STATE_CONTROL_CENTER_OPEN = !STATE_CONTROL_CENTER_OPEN;
      localStorage.setItem(CONFIG_CONTROL_OPEN_KEY, STATE_CONTROL_CENTER_OPEN ? 'open' : 'closed');
      renderStewardPanelContent();
    });
    var activityClearBtn = document.getElementById('nbf-activity-clear');
    if (activityClearBtn) activityClearBtn.addEventListener('click', function(e) {
      e.stopPropagation();
      clearActivityLog();
      addActivityLog('Activity log cleared.', 'warning');
      renderStewardPanelContent();
    });
    window.setTimeout(function() { refreshBackendStatus(false); }, 50);

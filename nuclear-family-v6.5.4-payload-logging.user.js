...
;(function (window, document) {
  ...
  function updateBackendStatusPanel() {
    ...
    if (checkBtn) checkBtn.addEventListener('click', function() { refreshBackendStatus(true); });
  }
  ...
  function drawStandingsTable(container, dataset, isPlaceholder) {
    ...
    var rowsHtml = dataset.map(function(driver) {
      ...
      var bestDisplay = driver.best === '—' ? '—' : (driver.best === 1 ? '🥇 P1' : 'P' + driver.best);
      rowHtml += '<td style="padding:8px 14px 8px 4px; color:' + mutedColor + '; text-align:center;">' + bestDisplay + '</td>';
      rowHtml += '</tr>';
      return rowHtml;
    }).join('');
  ...
  if (racingContext.isRacing) {
    ...
    if (isLogPage) {
      ...
      var runFocusedRaceScraper = function() {
      ...
      if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', runFocusedRaceScraper);
      } else {
        runFocusedRaceScraper();
      }
      window.addEventListener('focus', runFocusedRaceScraper);
      document.addEventListener('visibilitychange', runFocusedRaceScraper);
    } else {
      // Dashboard mode
      if (document.readyState === 'loading') {

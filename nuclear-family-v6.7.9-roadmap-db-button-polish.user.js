...
;(function (window, document) {
   ...
   *     BOOK 15 — Entry Point
   *
   * v6.7.9 Roadmap + Database Button Polish:
   *     Replaced paragraph roadmap with compact data tables and
   *     polished Database Tools delete button status badges.
   *
   * v6.7.8 League Setup Finish:
   *     Connected League Setup changes to Standings refresh, simplified
   *     selector wording to Series/Round/Event, and added View Standings flow.
   *
   * v6.7.7 Database Tools Operational:
   *     Added backend readiness states to delete buttons and made
   *     Database Maintenance buttons perform real safe checks/refreshes.
   *
   * v6.7.6 Backend Maintenance Alignment:
   *     Database Tools now align with Worker v1.0.13 reset/archive
   *     endpoints, including uploaded race result deletion.
   *
   * v6.7.5 Help Roadmap Priority Map:
   *     Updated Help Central roadmap with architecture map,
   *     readiness progress and prioritized finish checklist.
   *
   * v6.7.4 Status + Health + Archive Fix:
   *     Fixed launcher status for admin sessions, repaired System
   *     Health actions, and marked Archive as not installed.
   *
   * v6.7.3 Delete Tools Fallback:
   *     Added local cleanup fallback for Database Tools delete actions
   *     and clearer remote endpoint failure messaging.
   *
   * v6.7.2 Sync Progress Stability:
   *     Fixed sync progress state initialization, throttled progress
   *     rendering, and kept the original API sync pipeline intact.
   *
   * v6.6.20 Any Standings View Selector:
   *     Standings dropdown now supports all submitted races,
   *     competitions, championship seasons, and individual events.
   *
   * v6.6.19 Database Tools + Health Checklist Merge:
   *     Expanded Database Tools with diagnostics, health checklist,
   *     maintenance preview, and guarded development cleanup requests.
   *
   * v6.6.18 KISS Championship + Standings Fix:
   *     Simplified Championship Engine steps and stopped Standings
   *     competition selector reload loop.
   *
   * v6.6.17 Championship Button Bind Fix:
   *     Repaired Create Series/Create Round/Event click bindings and added
   *     console confirmation when championship buttons fire.
   *
   * v6.6.16 Championship Create Fix:
   *     Hardened season/event creation payloads and button bindings
   *     with config_json compatibility and clearer errors.
   *
   * v6.6.14 Competition Create Fix:
   *     Sends template default config/config_json when creating a
   *     competition so D1 required config_json is satisfied.
   *
   * v6.6.13 Series Standings Selector:
   *     Added Standings competition dropdown with all-races fallback
   *     and selected competition persistence.
   *
   * v6.6.12 Admin Database Tools:
   *     Added safe admin-only database diagnostics, local queue
   *     snapshot, and locked repair-helper placeholders.
   *
   * v6.6.7 Stability Pass:
   *     Hardened remaining render paths, idempotent queue bindings,
   *     safer standings rendering, and tightened UI/auth coupling.
   **********************************************************************/
  ...
  var CONFIG_STANDINGS_COMPETITION_KEY = 'nbf_v6613_standings_competition';
  ...
  var CF_COMPETITION_TEMPLATES_ENDPOINT = 'https://nuclear-family.selbaer.workers.dev/competition/templates';
  var CF_COMPETITIONS_ENDPOINT = 'https://nuclear-family.selbaer.workers.dev/competition/competitions';
  var CF_COMPETITION_PARTICIPANTS_ENDPOINT = 'https://nuclear-family.selbaer.workers.dev/competition/participants';
  var CF_COMPETITION_IMPORT_ENDPOINT = 'https://nuclear-family.selbaer.workers.dev/competition/participants/import';
  ...
  var CF_ADMIN_DEV_RESET_ENDPOINT = 'https://nuclear-family.selbaer.workers.dev/admin/dev-reset';
  var CF_ADMIN_ARCHIVE_ENDPOINT = 'https://nuclear-family.selbaer.workers.dev/admin/archive';
  ...
  var COMPETITION_SETUP_STATE = { templates: null, competitions: [], participants: [], message: '', loading: false };
  var STANDINGS_FILTER_STATE = {
    selectedCompetitionId: localStorage.getItem(CONFIG_STANDINGS_COMPETITION_KEY) || 'all',
    competitions: [],
    loadingSeries: false,
    loadedOnce: false,
    lastFetchAt: 0,
  ...
  var NBF_SINGLETON_EVENTS = {};

  function bindSingletonEvent(name, target, type, handler, options) {
    if (!name || !target || !type || typeof handler !== 'function') return;
    if (NBF_SINGLETON_EVENTS[name]) {
      try {
        NBF_SINGLETON_EVENTS[name].target.removeEventListener(
          NBF_SINGLETON_EVENTS[name].type,
          NBF_SINGLETON_EVENTS[name].handler,
          NBF_SINGLETON_EVENTS[name].options || false
        );
      } catch (err) {}
    }
    NBF_SINGLETON_EVENTS[name] = { target: target, type: type, handler: handler, options: options || false };
  ...
  function getStewardConfirmationIdentity() {
    if (BACKEND_LOGIN_IDENTITY && (BACKEND_LOGIN_IDENTITY.torn_id || BACKEND_LOGIN_IDENTITY.id)) {
      return String(BACKEND_LOGIN_IDENTITY.torn_id || BACKEND_LOGIN_IDENTITY.id);
    }
    try {
      var saved = sessionStorage.getItem('nbf_v6529_login_identity');
      if (saved) {
        var parsed = JSON.parse(saved);
        if (parsed && (parsed.torn_id || parsed.id)) return String(parsed.torn_id || parsed.id);
      }
    } catch (err) {}
    return 'anonymous';
  }

  function getFirstUploadConfirmKey() {
    return CONFIG_FIRST_UPLOAD_CONFIRM_KEY + '_' + getStewardConfirmationIdentity();
  }
  ...
  function hasConfirmedFirstUpload() {
    return secureGetValue(getFirstUploadConfirmKey(), '') === 'yes';
  }

  function confirmFirstRaceUpload(raceID, resultCount) {
    if (hasConfirmedFirstUpload()) return true;
    var identity = getStewardConfirmationIdentity();
    var message =
      'First upload confirmation for this steward.\n\n' +
      'Steward identity: ' + identity + '\n' +
      'Race ID: ' + raceID + '\n' +
      'Results: ' + resultCount + '\n\n' +
      'This race will be uploaded to the Nuclear Family League backend and associated with your logged-in steward/session identity.\n\n' +
      'This confirmation is remembered for THIS steward on this browser. Continue?';

    if (!window.confirm(message)) {
      setGateStatus('payload', 'upload cancelled by user');
      addActivityLog('Race #' + raceID + ': First upload confirmation cancelled for steward ' + identity + '.', 'warning');
      return false;
    }

    secureSetValue(getFirstUploadConfirmKey(), 'yes');
    addActivityLog('First upload confirmation accepted for steward ' + identity + '.', 'success');
    return true;
  }
  ...
  function processUploadQueue() {
    ...
    backendRequest(CF_INGEST_ENDPOINT, {
      ...
      .catch(function(err) {
        console.error('[NB Racing] Upload 500 / error details:', err);
  ...
  function renderSafeUploadQueueBody() {
    try {
      return renderUploadQueueBody();
    } catch (err) {
      console.error('[NB Racing] Upload queue render failed:', err);
      return '<div style="font-size:12px; color:#dc2626;">Upload Queue render failed: ' + escapeHTML(err && err.message ? err.message : String(err)) + '</div>';
    }
  }
  ...
  function renderUploadQueueBody() {
    ...
    try { queue = loadUploadQueue(); } catch (err) { queue = []; }
    try { history = loadUploadHistory(); } catch (err2) { history = []; }
    ...
    var html = '<div id="nbf-upload-queue-panel">' +
      ...
      renderStatModule('Pending', pending) +
      renderStatModule('Completed', completed) +
      renderStatModule('Failed', failed) +
      renderStatModule('Processing', UPLOAD_QUEUE_PROCESSING ? 'Yes' : 'No') +
      '</div>' +
      '<div style="display:flex; gap:8px; margin-bottom:10px; flex-wrap:wrap;">' +
      '<button id="nbf-upload-queue-process-btn" style="padding:6px 10px; background:#6366f1; color:#fff; border:none; border-radius:6px; font-size:11px; font-weight:600; cursor:pointer;">Process Queue Now</button>' +
    ...
    html += '<h4 style="margin:8px 0 4px 0; font-size:12px; color:var(--nbf-txt);">Current Queue</h4>';
    if (!queue.length) {
      html += '<div style="font-size:12px; color:var(--nbf-mut); padding:8px 0;">Queue is empty.</div>';
    } else {
      queue.slice(0, 8).forEach(function(q) {
        html += '<div style="display:grid; grid-template-columns:100px 90px 1fr 60px; gap:8px; padding:6px 0; border-bottom:1px solid var(--nbf-bld); font-size:11px;">' +
          '<div style="font-family:monospace;">#' + escapeHTML(q.race_id || '—') + '</div>' +
          '<div>' + uploadStateBadge(q.state || 'pending') + '</div>' +
          '<div style="color:var(--nbf-mut); overflow:hidden; text-overflow:ellipsis;">' + escapeHTML(q.last_error || q.fingerprint || '—') + '</div>' +
          '<div>×' + escapeHTML(q.attempts || 0) + '</div>' +
          '</div>';
      });
    }

    html += '<h4 style="margin:12px 0 4px 0; font-size:12px; color:var(--nbf-txt);">Recent History</h4>';
    if (!history.length) {
      html += '<div style="font-size:12px; color:var(--nbf-mut); padding:8px 0;">No uploads yet.</div>';
    } else {
      history.slice(0, 8).forEach(function(h) {
        html += '<div style="display:grid; grid-template-columns:70px 100px 90px 1fr; gap:8px; padding:5px 0; border-bottom:1px solid var(--nbf-bld); font-size:11px;">' +
          '<div style="color:var(--nbf-mut);">' + escapeHTML(h.at ? String(h.at).slice(0, 8) : '—') + '</div>' +
          '<div style="font-family:monospace;">#' + escapeHTML(h.race_id || '—') + '</div>' +
          '<div>' + uploadStateBadge(h.state || 'queued') + '</div>' +
  ...
  function renderActivityLogMarkup() {
    ...
    entries.slice(0, 12).forEach(function(entry) {
      ...
      html += '<div style="display:grid; grid-template-columns:70px 1fr; gap:8px; padding:4px 0; border-bottom:1px solid var(--nbf-bld); font-size:11px;">' +
        '<span style="color:var(--nbf-mut);">' + escapeHTML(entry.at || '') + '</span>' +
        '<span style="color:' + color + ';">' + escapeHTML(entry.message || '') + '</span>' +
        '</div>';
    });
    return html;
  }
  ...
  function renderBackendStatusMarkup() {
    ...
    var errorLine = BACKEND_STATUS.error
      ? '<div style="grid-column:1 / -1; color:#dc2626; font-size:11px; margin-top:4px;"><strong>Last Error:</strong> ' + escapeHTML(BACKEND_STATUS.error || '') + '</div>'
      : '';
    ...
    return '' +
      ...
      '    <div><strong style="color:var(--nbf-txt);">Backend:</strong> <span style="color:' + statusColor + '; font-weight:600;">' + statusIcon + ' ' + escapeHTML(BACKEND_STATUS.backend || '—') + latencyText + '</span></div>' +
      '    <div><strong style="color:var(--nbf-txt);">Database:</strong> ' + escapeHTML(BACKEND_STATUS.database || '—') + '</div>' +
      '    <div><strong style="color:var(--nbf-txt);">Worker Version:</strong> ' + escapeHTML(BACKEND_STATUS.version || '—') + '</div>' +
      '    <div><strong style="color:var(--nbf-txt);">Last Check:</strong> ' + escapeHTML(BACKEND_STATUS.lastCheck || '—') + '</div>' +
  ...
  function renderIngestionStatusMarkup(endpointStatus) {
    ...
    return '' +
      ...
      '    <div style="grid-column:1 / -1; border-top:1px solid var(--nbf-bld); padding-top:7px; margin-top:2px;"><strong style="color:var(--nbf-txt);">Last Upload:</strong> <span style="color:' + statusColor + '; font-weight:600;">' + statusIcon + ' ' + escapeHTML(status.message || '') + '</span></div>' +
      '    <div><strong style="color:var(--nbf-txt);">Race ID:</strong> ' + escapeHTML(status.race_id || '—') + '</div>' +
      '    <div><strong style="color:var(--nbf-txt);">Results:</strong> ' + escapeHTML(status.results || '—') + '</div>' +
      '    <div><strong style="color:var(--nbf-txt);">Time:</strong> ' + escapeHTML(status.at || '—') + '</div>' +
      ...
      .then(function(health) {
        return backendRequest(CF_STANDINGS_ENDPOINT, { method: 'GET' }, 5000)
          ...
          .then(function(standingsPayload) {
            ...
            updateBackendStatusPanel();
            if (typeof updateDatabaseToolsPanel === 'function') updateDatabaseToolsPanel();
            if (typeof window.NBF_UPDATE_LAUNCHER_STATUS === 'function') window.NBF_UPDATE_LAUNCHER_STATUS();
          });
      })
      .catch(function(err) {
  ...
  function mountFloatingInterface() {
    // v6.6.5: Racing-wide safe launcher.
    // Visible on every sid=racing page. Uses inline heading anchoring when safe,
    // otherwise a top-right fallback that never blocks chat.
    if (document.getElementById('nbf-action-trigger-root')) return;
    if (document.getElementById('nbf-action-trigger-fallback')) return;
    if (document.getElementById('nbf-action-trigger-button')) return;
    if (!document.body) return;

    function getLauncherStatus() {
      try {
        if (UPLOAD_QUEUE_PROCESSING) return { label: 'Uploading', color: '#2563eb', cls: 'uploading' };
      } catch (err1) {}

      try {
        if (STATE_SYNC_ACTIVE || (window.__NBF_SYNC_PROGRESS_STATE && window.__NBF_SYNC_PROGRESS_STATE.active)) {
          return { label: 'Syncing', color: '#d97706', cls: 'syncing' };
        }
      } catch (err2) {}

      try {
        if (BACKEND_LOGIN_IDENTITY && BACKEND_LOGIN_IDENTITY.role) {
          var roleLabel = String(BACKEND_LOGIN_IDENTITY.role).replace('_', ' ');
          return { label: roleLabel, color: '#16a34a', cls: 'logged-in' };
        }
      } catch (err3) {}

      try {
        if (STATE_ADMIN_TOKEN_SESSION) return { label: 'Admin', color: '#16a34a', cls: 'admin' };
      } catch (err4) {}

      try {
        if (BACKEND_STATUS && BACKEND_STATUS.state === 'online') {
          return { label: 'Online', color: '#16a34a', cls: 'online' };
        }
      } catch (err5) {}

      try {
        if (BACKEND_STATUS && BACKEND_STATUS.state === 'error') {
          return { label: 'Backend check', color: '#f59e0b', cls: 'check' };
        }
      } catch (err6) {}

      try {
        if (LAST_INGESTION_STATUS && LAST_INGESTION_STATUS.state === 'error') {
          return { label: 'Check', color: '#dc2626', cls: 'error' };
        }
      } catch (err7) {}

      return { label: 'Ready', color: '#16a34a', cls: 'ready' };
    }

    function updateLauncherStatus() {
      var btn = document.getElementById('nbf-action-trigger-button');
      if (!btn) return;

      var status = getLauncherStatus();
      var statusEl = btn.querySelector('.nbf-native-status-text');
      var dotEl = btn.querySelector('.nbf-native-dot');

      if (statusEl) statusEl.textContent = status.label;
      if (dotEl) {
        dotEl.style.background = status.color;
        dotEl.style.boxShadow = '0 0 0 1px ' + status.color + '33, 0 0 5px ' + status.color + '66';
      }

      btn.setAttribute('data-nbf-status', status.cls);
      btn.title = 'Nuclear Family Engine v' + ENGINE_VERSION + ' — ' + status.label;
    }

    window.NBF_UPDATE_LAUNCHER_STATUS = updateLauncherStatus;

    function openOrCloseNuclearFamily(e) {
      if (e) {
        e.preventDefault();
        e.stopPropagation();
      }
    ...
    function ensureLauncherCSS() {
      if (document.getElementById('nbf-native-launcher-style')) return;

      var style = document.createElement('style');
      style.id = 'nbf-native-launcher-style';
      style.textContent = ''
        + '#nbf-action-trigger-root{'
        + 'display:inline-flex;align-items:center;gap:6px;margin-left:12px;vertical-align:middle;font-family:inherit;position:relative;z-index:20;'
        + '}'
        + '#nbf-action-trigger-button{'
        + 'display:inline-flex!important;align-items:center!important;gap:7px!important;height:32px!important;min-width:108px!important;'
        + 'box-sizing:border-box!important;padding:3px 9px 3px 5px!important;margin:0!important;'
        + 'border-radius:20px!important;border:1px solid rgba(0,0,0,.14)!important;'
        + 'background:rgba(0,0,0,.05)!important;'
        + 'color:#2f3742!important;font-family:inherit!important;cursor:pointer!important;vertical-align:middle!important;'
        + 'box-shadow:none!important;text-align:left!important;transition:background .14s ease,border-color .14s ease,box-shadow .14s ease,filter .14s ease!important;'
        + '}'
        + '#nbf-action-trigger-button .nbf-native-icon{'
        + 'display:flex;align-items:center;justify-content:center;width:24px;height:24px;border-radius:50%;'
        + 'background:rgba(255,255,255,.72);color:#1f2937;font-size:14px;font-weight:900;'
        + 'box-shadow:inset 0 1px 0 rgba(255,255,255,.8),0 1px 2px rgba(0,0,0,.18);'
        + '}'
        + '#nbf-action-trigger-button .nbf-native-copy{display:flex;flex-direction:column;align-items:flex-start;line-height:1.05;}'
        + '#nbf-action-trigger-button .nbf-native-title{font-size:11px;font-weight:800;color:#27313d;white-space:nowrap;}'
        + '#nbf-action-trigger-button .nbf-native-status{display:flex;align-items:center;gap:4px;margin-top:2px;font-size:9px;font-weight:700;color:#12813a;white-space:nowrap;}'
        + '#nbf-action-trigger-button .nbf-native-dot{width:6px;height:6px;border-radius:50%;background:#16a34a;box-shadow:0 0 0 1px rgba(22,163,74,.22),0 0 5px rgba(22,163,74,.45);}'
        + '#nbf-action-trigger-button:hover{background:rgba(0,0,0,.075)!important;border-color:rgba(0,0,0,.22)!important;box-shadow:0 1px 2px rgba(0,0,0,.10)!important;filter:brightness(1.03)!important;}'
        + '#nbf-action-trigger-button:active{filter:brightness(.98)!important;}'
        + '#nbf-action-trigger-button[data-nbf-status="offline"] .nbf-native-status,'
        + '#nbf-action-trigger-button[data-nbf-status="error"] .nbf-native-status{color:#dc2626!important;}'
        + '#nbf-action-trigger-button[data-nbf-status="syncing"] .nbf-native-status{color:#d97706!important;}'
        + '#nbf-action-trigger-button[data-nbf-status="uploading"] .nbf-native-status{color:#2563eb!important;}'
        + '#nbf-action-trigger-fallback{position:fixed!important;top:74px!important;right:18px!important;z-index:999999!important;}';

      document.head.appendChild(style);
    }

    function buildLauncherRoot(fallbackMode) {
      ensureLauncherCSS();

      var root = document.createElement('span');
      root.id = fallbackMode ? 'nbf-action-trigger-fallback' : 'nbf-action-trigger-root';

      var capsule = document.createElement('button');
      capsule.id = 'nbf-action-trigger-button';
      capsule.type = 'button';
      capsule.innerHTML =
        '<span class="nbf-native-icon">⚡</span>' +
        '<span class="nbf-native-copy">' +
        '  <span class="nbf-native-title">NF Engine</span>' +
        '  <span class="nbf-native-status"><span class="nbf-native-dot"></span><span class="nbf-native-status-text">Connected</span></span>' +
        '</span>';

      capsule.addEventListener('click', openOrCloseNuclearFamily);
      root.appendChild(capsule);
      return root;
    }

    function isUsefulAnchor(el) {
      if (!el || el === document.body || !el.offsetParent) return false;
      if (el.closest && el.closest('#nbf-action-trigger-root,#nbf-action-trigger-fallback,#nbf-backdrop-mask')) return false;
      return true;
    }

    function findRacingAnchor() {
      ...
      for (var i = 0; i < headings.length; i++) {
        ...
        if ((headingText === 'torn city raceway' || headingText.indexOf('torn city raceway') === 0) && isUsefulAnchor(headings[i])) {
          return headings[i];
        }
      }

      var titleCandidates = Array.prototype.slice.call(document.querySelectorAll('[class*="title" i], [class*="heading" i]'));
      for (var j = 0; j < titleCandidates.length; j++) {
        var el = titleCandidates[j];
        if (!isUsefulAnchor(el) || el.children.length > 4) continue;
        ...
        if (
          t.indexOf('race') !== -1 ||
          t.indexOf('racing') !== -1 ||
          t.indexOf('official events') !== -1 ||
          t.indexOf('custom events') !== -1 ||
          t.indexOf('your cars') !== -1 ||
          t.indexOf('parts') !== -1 ||
          t.indexOf('statistics') !== -1
        ) {
          return el;
        }
      }
    ...
    function mountLauncher() {
      if (document.getElementById('nbf-action-trigger-root') || document.getElementById('nbf-action-trigger-fallback')) {
        updateLauncherStatus();
        return true;
      }

      var anchor = findRacingAnchor();

      if (anchor) {
        try {
          var inlineRoot = buildLauncherRoot(false);
          anchor.appendChild(inlineRoot);
          updateLauncherStatus();
          return true;
        } catch (err) {}
      }

      var fallbackRoot = buildLauncherRoot(true);
    ...
    var timer = window.setInterval(function() {
      ...
      var current = document.getElementById('nbf-action-trigger-root') || document.getElementById('nbf-action-trigger-fallback');
      var anchor = findRacingAnchor();

      if (anchor && current && current.parentNode !== anchor && !document.getElementById('nbf-backdrop-mask')) {
        try {
          current.parentNode.removeChild(current);
          current.id = 'nbf-action-trigger-root';
    ...
    window.setInterval(updateLauncherStatus, 2000);
  }
...
document.getElementById('nbf-cfg-type').addEventListener('change', function() {
    ...
    bindSingletonEvent('setupOutsideClick', document, 'click', function(e) {
  ...
  function nbfSeriesLabel(raw) {
    raw = String(raw || '').trim();
    return raw || 'Unnamed Series';
  }

  function nbfRoundLabel(raw) {
    raw = String(raw || '').trim();
    return raw || 'Unnamed Round/Event';
  }

  function refreshLeagueSetupAndStandings(options) {
    options = options || {};
    STANDINGS_FILTER_STATE.loadedOnce = false;
    var promises = [];

    try {
      var p1 = fetchChampionshipData();
      if (p1 && typeof p1.then === 'function') promises.push(p1);
    } catch (err1) {}

    try {
      var p2 = fetchCompetitionSetupData();
      if (p2 && typeof p2.then === 'function') promises.push(p2);
    } catch (err2) {}

    window.setTimeout(function() {
      try { loadStandingsCompetitionOptions(true); } catch (err3) {}
      if (options.openStandings) {
        STATE_ACTIVE_TAB = 'standings';
        try { switchViewRoute('standings'); } catch (err4) { renderStandingsPanelContent(); }
      } else if (STATE_ACTIVE_TAB === 'standings') {
        try { renderStandingsPanelContent(); } catch (err5) {}
      }
    }, 250);
  ...
  function setStandingsScopeAndOpen(scopeValue) {
    STANDINGS_FILTER_STATE.selectedCompetitionId = String(scopeValue || 'all');
    localStorage.setItem(CONFIG_STANDINGS_COMPETITION_KEY, STANDINGS_FILTER_STATE.selectedCompetitionId);
    STATE_ACTIVE_TAB = 'standings';
    try { switchViewRoute('standings'); } catch (err) { renderStandingsPanelContent(); }
  }


  function getStandingsScopeOptions() {
    var options = [];
    var seen = {};
    ...
    ((CHAMPIONSHIP_STATE && CHAMPIONSHIP_STATE.seasons) || []).forEach(function(s) {
      if (!s || !s.id) return;
      addOption({
        value: 'season:' + String(s.id),
        type: 'season',
        group: 'Series',
        id: String(s.id),
        label: 'Series: ' + nbfSeriesLabel(s.name) + (s.year ? ' (' + s.year + ')' : '')
      });
    });

    ((CHAMPIONSHIP_STATE && CHAMPIONSHIP_STATE.events) || []).forEach(function(e) {
      if (!e || !e.id) return;
      addOption({
        value: 'event:' + String(e.id),
        type: 'event',
        group: 'Rounds / Events',
        id: String(e.id),
        season_id: e.season_id ? String(e.season_id) : '',
        label: 'Round/Event: ' + (e.season_name ? nbfSeriesLabel(e.season_name) + ' · ' : '') + 'R' + (e.round_number || '?') + ' · ' + nbfRoundLabel(e.name)
      });
    });

    (STANDINGS_FILTER_STATE.competitions || []).forEach(function(c) {
      if (!c || !c.id) return;
      addOption({
        value: 'competition:' + String(c.id),
        type: 'competition',
        group: 'Templates / Legacy Series',
        id: String(c.id),
        label: 'Series Template: ' + nbfSeriesLabel(c.name) + (c.template_type ? ' · ' + String(c.template_type).toUpperCase() : '') + (c.status ? ' · ' + c.status : '')
      });
    });
  ...
  function parseStandingsScope(value) {
    value = String(value || 'all');
    if (value === 'all') return { type: 'all', id: '', value: 'all', label: 'All Submitted Races' };

    var parts = value.split(':');
    if (parts.length < 2) return { type: 'all', id: '', value: 'all', label: 'All Submitted Races' };

    var type = parts[0];
    var id = parts.slice(1).join(':');
    var opts = getStandingsScopeOptions();
    ...
    return {
      ...
      id: id,
      value: value,
      label: (type === 'season' ? 'Series' : (type === 'event' ? 'Round/Event' : 'Series Template')) + ' #' + id
    };
  }

  function normalizeCompetitionListResponse(payload) {
    if (!payload) return [];
  ...
  function loadStandingsCompetitionOptions(force) {
    if (STANDINGS_FILTER_STATE.loadingSeries) return;

    if (COMPETITION_SETUP_STATE && Array.isArray(COMPETITION_SETUP_STATE.competitions) && COMPETITION_SETUP_STATE.competitions.length) {
      STANDINGS_FILTER_STATE.competitions = COMPETITION_SETUP_STATE.competitions.map(function(c) {
        return {
          id: String(c.id || c.competition_id || ''),
          name: String(c.name || c.title || ('Competition #' + (c.id || c.competition_id || ''))),
          template_type: String(c.template_type || c.type || ''),
          status: String(c.status || '')
        };
      }).filter(function(c) { return c.id; });
      STANDINGS_FILTER_STATE.loadedOnce = true;
    }

    if (isAdminUnlocked() && CHAMPIONSHIP_STATE && !CHAMPIONSHIP_STATE.loading && !CHAMPIONSHIP_STATE.message && !(CHAMPIONSHIP_STATE.seasons || []).length && !(CHAMPIONSHIP_STATE.events || []).length) {
      window.setTimeout(fetchChampionshipData, 50);
    }

    var now = Date.now();
    if (!force && STANDINGS_FILTER_STATE.loadedOnce) return;
    if (!force && STANDINGS_FILTER_STATE.lastFetchAt && now - STANDINGS_FILTER_STATE.lastFetchAt < 30000) return;

    STANDINGS_FILTER_STATE.loadingSeries = true;
    STANDINGS_FILTER_STATE.lastFetchAt = now;

    backendRequest(CF_COMPETITIONS_ENDPOINT, {
      method: 'GET',
      headers: STATE_ADMIN_TOKEN_SESSION ? { 'Authorization': 'Bearer ' + STATE_ADMIN_TOKEN_SESSION } : {}
    }, 8000)
      .then(function(res) {
        return res.json().then(function(body) {
          if (!res.ok || (body && body.ok === false)) throw new Error(body && body.error ? body.error : 'HTTP ' + res.status);

          STANDINGS_FILTER_STATE.competitions = normalizeCompetitionListResponse(body).map(function(c) {
            return {
              id: String(c.id || c.competition_id || ''),
              name: String(c.name || c.title || ('Competition #' + (c.id || c.competition_id || ''))),
              template_type: String(c.template_type || c.type || ''),
          ...
          STANDINGS_FILTER_STATE.loadedOnce = true;
          STANDINGS_FILTER_STATE.message = getStandingsScopeOptions().length ? 'Scope list loaded.' : 'No series or round/events found yet.';
        });
      })
      .catch(function(err) {
        STANDINGS_FILTER_STATE.loadedOnce = true;
        STANDINGS_FILTER_STATE.message = getStandingsScopeOptions().length ? 'Using loaded seasons/events. Competition endpoint unavailable.' : 'Scope list unavailable. Create a season/event or refresh after creating one.';
        console.warn('[NB Racing] Standings scope list unavailable:', err);
      })
      .finally(function() {
        STANDINGS_FILTER_STATE.loadingSeries = false;

        if (force && document.getElementById('nbf-layout-body')) {
          renderStandingsPanelContent();
          return;
        }

        var bar = document.getElementById('nbf-standings-filter-bar');
        if (bar) {
          var tmp = document.createElement('div');
          tmp.innerHTML = renderStandingsCompetitionSelector();
          if (tmp.firstChild) {
            bar.parentNode.replaceChild(tmp.firstChild, bar);
            bindStandingsCompetitionSelector();
          }
        }
      });
  }

  function getSelectedStandingsCompetition() {
    return parseStandingsScope(STANDINGS_FILTER_STATE.selectedCompetitionId || 'all');
  }

  function buildStandingsEndpoint() {
    var selected = getSelectedStandingsCompetition();
    if (!selected || selected.type === 'all' || !selected.id) return CF_STANDINGS_ENDPOINT;

    var base = CF_CHAMPIONSHIP_STANDINGS_ENDPOINT || CF_STANDINGS_ENDPOINT;
    var sep = base.indexOf('?') === -1 ? '?' : '&';

    if (selected.type === 'competition') return base + sep + 'competition_id=' + encodeURIComponent(selected.id);
    if (selected.type === 'season') return base + sep + 'season_id=' + encodeURIComponent(selected.id);
    if (selected.type === 'event') return base + sep + 'event_id=' + encodeURIComponent(selected.id);

    return CF_STANDINGS_ENDPOINT;
  }

  function renderStandingsCompetitionSelector() {
    var selectedValue = String(STANDINGS_FILTER_STATE.selectedCompetitionId || 'all');
    var scopeOptions = getStandingsScopeOptions();

    var options = '<option value="all"' + (selectedValue === 'all' ? ' selected' : '') + '>All Submitted Races</option>';

    var groups = {
      'Series': [],
      'Rounds / Events': [],
      'Templates / Legacy Series': []
    };

    scopeOptions.forEach(function(opt) {
      var group = opt.group || 'Series';
      ...
      return '<optgroup label="' + escapeHTML(label) + '">' +
        items.map(function(opt) {
          var cleanLabel = opt.label
            .replace(/^Series: /, '')
            .replace(/^Round\/Event: /, '')
            .replace(/^Series Template: /, '');
          return '<option value="' + escapeHTML(opt.value) + '"' + (opt.value === selectedValue ? ' selected' : '') + '>' + escapeHTML(cleanLabel) + '</option>';
        }).join('') +
      '</optgroup>';
    }

    options += renderGroup('Series', groups['Series']);
    options += renderGroup('Rounds / Events', groups['Rounds / Events']);
    options += renderGroup('Templates / Legacy Series', groups['Templates / Legacy Series']);

    var selected = getSelectedStandingsCompetition();
    var subtitle = selected && selected.type !== 'all'
      ? selected.label.replace(/^Series: |^Round\/Event: |^Series Template: /, '')
      : 'All backend race results';

    var emptyMsg = !scopeOptions.length
      ? '<div style="flex-basis:100%; font-size:11px; color:#b45309;">No Series or Round/Event found yet. Create one in League Setup, then press Refresh.</div>'
      : '';

    return '' +
      '<div id="nbf-standings-filter-bar" style="display:flex; align-items:center; justify-content:space-between; gap:12px; flex-wrap:wrap; padding:10px 16px; border-bottom:1px solid var(--nbf-bld); background:var(--nbf-alt2);">' +
      '  <div style="display:flex; flex-direction:column; gap:3px;">' +
      '    <strong style="font-size:12px; color:var(--nbf-txt);">Standings</strong>' +
      '    <span style="font-size:11px; color:var(--nbf-mut);">' + escapeHTML(subtitle) + '</span>' +
      '  </div>' +
      '  <div style="display:flex; align-items:center; gap:8px; flex-wrap:wrap;">' +
      '    <select id="nbf-standings-competition-select" style="min-width:320px; padding:6px 9px; background:var(--nbf-field-bg); color:var(--nbf-txt); border:1px solid var(--nbf-btn-border); border-radius:6px; font-size:12px;">' + options + '</select>' +
      '    <button id="nbf-standings-competition-refresh" style="padding:6px 10px; background:var(--nbf-field-bg); color:var(--nbf-txt); border:1px solid var(--nbf-btn-border); border-radius:6px; font-size:11px; cursor:pointer;">Refresh</button>' +
      '  </div>' +
      emptyMsg +
      (STANDINGS_FILTER_STATE.message ? '<div style="flex-basis:100%; font-size:11px; color:var(--nbf-mut);">' + escapeHTML(STANDINGS_FILTER_STATE.message) + '</div>' : '') +
      '</div>';
  }

  function bindStandingsCompetitionSelector() {
    var select = document.getElementById('nbf-standings-competition-select');
    if (select) nbfBindOnce(select, 'change', function() {
      STANDINGS_FILTER_STATE.selectedCompetitionId = this.value || 'all';
      localStorage.setItem(CONFIG_STANDINGS_COMPETITION_KEY, STANDINGS_FILTER_STATE.selectedCompetitionId);
      renderStandingsPanelContent();
    }, 'standingsScopeSelect');

    var refreshBtn = document.getElementById('nbf-standings-competition-refresh');
    if (refreshBtn) nbfBindOnce(refreshBtn, 'click', function() {
      STANDINGS_FILTER_STATE.loadedOnce = false;
      fetchChampionshipData();
      refreshLeagueSetupAndStandings();
    }, 'standingsScopeRefresh');
  }


  function renderStandingsPanelContent() {
    function safeNormalizeStandingsDataset(input) {
      try {
        return normalizeStandingsDataset(input);
      } catch (err) {
        console.error('[NB Racing] Standings normalization failed:', err);
        try { return normalizeStandingsDataset(STANDINGS_PLACEHOLDER || []); }
    ...
    loadStandingsCompetitionOptions(false);

    var selectedCompetition = getSelectedStandingsCompetition();
    var endpoint = buildStandingsEndpoint();
    var isPlaceholder = endpoint.indexOf('YOUR-WORKER') !== -1;

    targetContainer.innerHTML =
      '<div style="display:flex; flex-direction:column; min-height:100%; background:var(--nbf-main);">' +
      renderStandingsCompetitionSelector() +
    ...
    if (isPlaceholder) {
      ...
      backendRequest(endpoint, { method: 'GET' }, 5000)
      .then(function(res) {
        if (!res.ok) throw new Error('HTTP ' + res.status);
        return res.json();
      })
      .then(function(data) {
        var normalizedDataset = safeNormalizeStandingsDataset(data);
        drawStandingsTable(targetContainer, normalizedDataset, false);
      })
      .catch(function(err) {
        console.error('[NB Racing] Standings fetch error:', err);

        var selectedNotice = selectedCompetition
          ? 'Selected scope endpoint failed. The backend may not support this standings filter yet.'
          : 'All-races standings failed to load.';
        ...
        targetContainer.innerHTML =
          ...
          '  <div style="font-size:11px; color:var(--nbf-mut);">' + escapeHTML(err.message || String(err)) + '</div>' +
          '  <div style="font-size:11px; color:var(--nbf-mut);">' + escapeHTML(selectedNotice) + '</div>' +
  ...
  function drawStandingsTable(container, dataset, isPlaceholder) {
    if (!document.getElementById('nbf-standings-sticky-style')) {
      var standingsStickyStyle = document.createElement('style');
      standingsStickyStyle.id = 'nbf-standings-sticky-style';
      standingsStickyStyle.textContent =
        '#nbf-standings-scroll-frame{' +
        'max-height:calc(100vh - 310px);min-height:260px;overflow:auto;position:relative;background:var(--nbf-main);' +
        '}' +
        '#nbf-standings-table thead{position:sticky;top:0;z-index:50;}' +
        '#nbf-standings-table thead th,' +
        '#nbf-standings-panel thead th,' +
        '.nbf-standings-table thead th{' +
        'position:sticky!important;top:0!important;z-index:51!important;background:var(--nbf-alt,#f1f5f9)!important;' +
        'box-shadow:0 2px 0 var(--nbf-bld,#d7dde5),0 3px 8px rgba(0,0,0,.08)!important;' +
        '}' +
        '#nbf-standings-table{border-collapse:separate!important;border-spacing:0!important;}';
      document.head.appendChild(standingsStickyStyle);
    }
    ...
    // Summary stat cards
    var totalRaces = dataset.length ? Math.max.apply(Math, dataset.map(function(row) { return Number(row.races || 0); }).filter(function(v) { return Number.isFinite(v); })) : 0;
    ...
    var rowsHtml = dataset.map(function(driver) {
      ...
      rowHtml += '<td style="padding:8px 4px; font-weight:600; color:' + anchorColor + ';">' + escapeHTML(driver.name || '') + '</td>';
      rowHtml += '<td style="padding:8px 4px; color:' + mutedColor + '; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; max-width:130px;" title="' + escapeHTML(driver.faction || '') + '">' + escapeHTML(driver.faction || '') + '</td>';
    ...
    container.innerHTML =
      ...
      renderStandingsCompetitionSelector() +
      summaryHtml +
      '<div id="nbf-standings-scroll-frame" style="overflow:auto; max-height:calc(100vh - 310px); min-height:260px; flex:1; background:var(--nbf-main); position:relative;">' +
      tableHtml +
      '</div>' +
      '</div>';
    bindStandingsCompetitionSelector();
  ...
  function renderControlAccordion(sectionId, title, subtitle, bodyHtmlOrRenderer, defaultOpen) {
    var isOpen = isControlSectionOpen(sectionId, defaultOpen);
    var safeSectionId = escapeHTML(sectionId || '');
    ...
    if (isOpen) {
      try {
        bodyHtml = (typeof bodyHtmlOrRenderer === 'function') ? bodyHtmlOrRenderer() : (bodyHtmlOrRenderer || '');
      } catch (err) {
        console.error('[NB Racing] Accordion body render failed:', sectionId, err);
        bodyHtml = '<div style="font-size:12px; color:#dc2626; padding:10px;">Section failed to render: ' + escapeHTML(err && err.message ? err.message : String(err)) + '</div>';
      }
    }

    return '' +
      '<div class="nbf-control-accordion" data-nbf-section="' + safeSectionId + '" style="background:var(--nbf-alt2); border:1px solid var(--nbf-bld); border-radius:8px; margin-bottom:10px; overflow:hidden;">' +
      '  <div class="nbf-control-accordion-toggle" data-nbf-section="' + safeSectionId + '" style="padding:11px 14px; cursor:pointer; display:flex; justify-content:space-between; align-items:center; gap:12px; background:var(--nbf-alt);">' +
      '    <strong style="font-size:13px; color:var(--nbf-txt);">' + (isOpen ? '▲ ' : '▼ ') + safeTitle + '</strong>' +
      '    <span style="font-size:11px; color:var(--nbf-mut); text-align:right; max-width:55%; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;">' + safeSubtitle + '</span>' +
      '  </div>' +
      '  <div class="nbf-control-accordion-body" data-nbf-section="' + safeSectionId + '" style="display:' + (isOpen ? 'block' : 'none') + '; padding:16px;">' +
      bodyHtml +
      '  </div>' +
      '</div>';
  }
  ...
  function renderAdminSystemHealthBody() {
    var statusColor = BACKEND_STATUS.state === 'online' ? '#16a34a' : (BACKEND_STATUS.state === 'error' ? '#dc2626' : '#64748b');
    var roleText = BACKEND_LOGIN_IDENTITY
      ? ((BACKEND_LOGIN_IDENTITY.display_name || BACKEND_LOGIN_IDENTITY.name || BACKEND_LOGIN_IDENTITY.torn_id || 'logged in') + ' · ' + (BACKEND_LOGIN_IDENTITY.role || 'role unknown'))
      : 'Not logged in';

    return '' +
      '<div style="display:grid; grid-template-columns:repeat(2,minmax(0,1fr)); gap:7px 16px; font-size:12px; color:var(--nbf-mut);">' +
      '  <div><strong style="color:var(--nbf-txt);">Userscript:</strong> v' + escapeHTML(ENGINE_VERSION) + '</div>' +
      '  <div><strong style="color:var(--nbf-txt);">Backend:</strong> <span style="color:' + statusColor + '; font-weight:700;">● ' + escapeHTML(BACKEND_STATUS.backend || 'Not checked') + '</span></div>' +
      '  <div><strong style="color:var(--nbf-txt);">Database:</strong> ' + escapeHTML(BACKEND_STATUS.database || '—') + '</div>' +
      '  <div><strong style="color:var(--nbf-txt);">Worker Version:</strong> ' + escapeHTML(BACKEND_STATUS.version || '—') + '</div>' +
      '  <div><strong style="color:var(--nbf-txt);">Latency:</strong> ' + (BACKEND_STATUS.latency !== null ? escapeHTML(BACKEND_STATUS.latency + ' ms') : '—') + '</div>' +
      '  <div><strong style="color:var(--nbf-txt);">Admin Session:</strong> ' + escapeHTML(roleText) + '</div>' +
      '  <div style="grid-column:1 / -1;"><strong style="color:var(--nbf-txt);">Last Check:</strong> ' + escapeHTML(BACKEND_STATUS.lastCheck || '—') + '</div>' +
      (BACKEND_STATUS.error ? '  <div style="grid-column:1 / -1; color:#dc2626;"><strong>Last Error:</strong> ' + escapeHTML(BACKEND_STATUS.error) + '</div>' : '') +
      '</div>' +
      '<div style="margin-top:12px; display:flex; gap:8px; flex-wrap:wrap;">' +
      '  <button id="nbf-system-health-run-btn" style="padding:7px 12px; background:#6366f1; color:#fff; border:none; border-radius:6px; font-size:12px; font-weight:600; cursor:pointer;">Run System Check</button>' +
      '  <button id="nbf-system-health-database-btn" style="padding:7px 12px; background:var(--nbf-field-bg); color:var(--nbf-txt); border:1px solid var(--nbf-btn-border); border-radius:6px; font-size:12px; cursor:pointer;">Open Database Tools</button>' +
      '</div>';
  }
  ...
  function canManageBackendStewards() {
    if (!BACKEND_LOGIN_IDENTITY) return false;
    var role = String(BACKEND_LOGIN_IDENTITY.role || '');
  ...
  function renderLoggedInIdentityCard() {
    if (!BACKEND_LOGIN_IDENTITY) return '';
    var displayName = BACKEND_LOGIN_IDENTITY.display_name || BACKEND_LOGIN_IDENTITY.name || 'Logged-in steward';
    var tornId = BACKEND_LOGIN_IDENTITY.torn_id || BACKEND_LOGIN_IDENTITY.id || '—';
    var role = BACKEND_LOGIN_IDENTITY.role || 'steward';
    return '' +
      '<div style="margin:10px 0; padding:10px 12px; border:1px solid #16a34a55; background:#dcfce7; border-radius:8px;">' +
      '  <div style="font-size:12px; font-weight:800; color:#166534;">● Logged in as ' + escapeHTML(displayName) + '</div>' +
      '  <div style="font-size:11px; color:#166534; margin-top:3px;">Torn ID: ' + escapeHTML(tornId) + ' · Role: ' + escapeHTML(role) + '</div>' +
      '</div>';
  }
  ...
  function getDatabaseToolsLocalSnapshot() {
    var queue = [];
    var history = [];
    var activity = [];
    var submitted = {};
    ...
    try { queue = loadUploadQueue(); } catch (err) { queue = []; }
    try { history = loadUploadHistory(); } catch (err2) { history = []; }
    try { activity = loadActivityLog(); } catch (err3) { activity = []; }
    try { submitted = loadSubmittedRaceMap(); } catch (err4) { submitted = {}; }

    var seasons = (CHAMPIONSHIP_STATE && CHAMPIONSHIP_STATE.seasons) || [];
    var events = (CHAMPIONSHIP_STATE && CHAMPIONSHIP_STATE.events) || [];
    var unassigned = (CHAMPIONSHIP_STATE && CHAMPIONSHIP_STATE.unassigned) || [];
    var standings = (CHAMPIONSHIP_STATE && CHAMPIONSHIP_STATE.standings) || [];
    var competitions = (COMPETITION_SETUP_STATE && COMPETITION_SETUP_STATE.competitions) || [];
    var participants = (COMPETITION_SETUP_STATE && COMPETITION_SETUP_STATE.participants) || [];

    return {
      queue_total: queue.length,
      queue_pending: queue.filter(function(q) { return q.state !== 'completed' && q.state !== 'failed_permanent'; }).length,
      queue_completed: queue.filter(function(q) { return q.state === 'completed'; }).length,
      queue_failed: queue.filter(function(q) { return q.state === 'failed_permanent'; }).length,
      history_total: history.length,
      submitted_total: Object.keys(submitted || {}).length,
      activity_total: activity.length,
      seasons_total: seasons.length,
      events_total: events.length,
      unassigned_total: unassigned.length,
      standings_total: standings.length,
      competitions_total: competitions.length,
      participants_total: participants.length,
      latest_queue: queue.slice(0, 5),
      latest_history: history.slice(0, 5),
      latest_activity: activity.slice(0, 5)
    };
  }

  function renderDbToolCard(title, value, detail) {
    return '' +
      '<div style="background:var(--nbf-main); border:1px solid var(--nbf-bld); border-radius:8px; padding:10px 12px;">' +
      '  <div style="font-size:10px; color:var(--nbf-mut); text-transform:uppercase; font-weight:700; letter-spacing:.04em;">' + escapeHTML(title) + '</div>' +
      '  <div style="font-size:18px; font-weight:800; color:var(--nbf-txt); margin-top:3px;">' + escapeHTML(value) + '</div>' +
      (detail ? '  <div style="font-size:11px; color:var(--nbf-mut); margin-top:3px;">' + escapeHTML(detail) + '</div>' : '') +
      '</div>';
  }

  function renderDbToolsMiniRows(items, emptyText) {
    if (!items || !items.length) {
      return '<div style="font-size:12px; color:var(--nbf-mut); padding:8px 0;">' + escapeHTML(emptyText || 'No data.') + '</div>';
    }

    return items.map(function(item) {
      var race = item.race_id || item.raceID || '—';
      var state = item.state || item.type || 'info';
      var msg = item.message || item.last_error || item.fingerprint || item.at || '—';
      return '<div style="display:grid; grid-template-columns:90px 90px 1fr; gap:8px; padding:5px 0; border-bottom:1px solid var(--nbf-bld); font-size:11px;">' +
        '<div style="font-family:monospace; color:var(--nbf-txt);">#' + escapeHTML(race) + '</div>' +
        '<div>' + uploadStateBadge(state) + '</div>' +
        '<div style="color:var(--nbf-mut); overflow:hidden; text-overflow:ellipsis; white-space:nowrap;">' + escapeHTML(msg) + '</div>' +
      '</div>';
    }).join('');
  }

  function renderDbToolsLockedAction(label, reason) {
    var actionId = 'nbf-db-maint-' + String(label || '').toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '');
    return '<button id="' + escapeHTML(actionId) + '" type="button" title="' + escapeHTML(reason || '') + '" style="padding:7px 10px; background:var(--nbf-field-bg); color:var(--nbf-txt); border:1px solid var(--nbf-btn-border); border-radius:6px; font-size:11px; cursor:pointer;">🔧 ' + escapeHTML(label) + '</button>';
  }

  
  function getDatabaseToolsBackendReadiness() {
    var health = DATABASE_TOOLS_STATE.health || {};
    var config = DATABASE_TOOLS_STATE.config || {};
    var archive = DATABASE_TOOLS_STATE.archive || {};
    var devReset = DATABASE_TOOLS_STATE.devReset || {};

    return {
      health_ok: health.ok === true,
      config_ok: config.ok === true || config.overall === 'ready',
      archive_ok: archive.ok === true && archive.installed === true,
      dev_reset_known: devReset.ok === true || devReset.http_status === 400 || devReset.http_status === 401 || devReset.http_status === 403,
      authorized: !!BACKEND_LOGIN_IDENTITY && String(BACKEND_LOGIN_IDENTITY.role || '') === 'super_admin'
    };
  }

  function renderDbToolsButtonStatus(scope) {
    var ready = getDatabaseToolsBackendReadiness();
    var localOnly = scope === 'queue' || scope === 'submitted';

    if (localOnly) {
      return { label: 'Local', bg: '#eef2ff', fg: '#3730a3', border: '#c7d2fe' };
    }

    if (!ready.authorized) {
      return { label: 'Super admin', bg: '#f1f5f9', fg: '#475569', border: '#cbd5e1' };
    }

    if (ready.dev_reset_known) {
      return { label: 'Backend ready', bg: '#ecfdf5', fg: '#047857', border: '#a7f3d0' };
    }

    return { label: 'Verify backend', bg: '#fffbeb', fg: '#92400e', border: '#fde68a' };
  }

  function runDatabaseMaintenanceAction(action) {
    DATABASE_TOOLS_STATE.loading = true;
    DATABASE_TOOLS_STATE.message = 'Running maintenance: ' + action + '...';
    updateDatabaseToolsPanel();

    if (action === 'refresh-cache') {
      DATABASE_TOOLS_STATE.local = getDatabaseToolsLocalSnapshot();
      STANDINGS_FILTER_STATE.loadedOnce = false;
      fetchChampionshipData();
      fetchCompetitionSetupData();
      loadStandingsCompetitionOptions(true);
      DATABASE_TOOLS_STATE.loading = false;
      DATABASE_TOOLS_STATE.message = 'Cache refreshed: local snapshot, series choices and standings data reloaded.';
      DATABASE_TOOLS_STATE.lastRun = formatBackendCheckTime();
      updateDatabaseToolsPanel();
      showScraperToast('Cache refreshed.', 'success', 2500);
      return;
    }

    if (action === 'verify-assignments') {
      var local = getDatabaseToolsLocalSnapshot();
      var issues = [];
      if (local.unassigned_total > 0) issues.push(local.unassigned_total + ' unassigned uploaded race(s)');
      if (local.events_total === 0) issues.push('no rounds/events loaded');
      if (local.seasons_total === 0) issues.push('no series loaded');
      DATABASE_TOOLS_STATE.loading = false;
      DATABASE_TOOLS_STATE.message = issues.length
        ? 'Assignment check finished: ' + issues.join(', ') + '.'
        : 'Assignment check finished: no obvious local assignment issues.';
      DATABASE_TOOLS_STATE.lastRun = formatBackendCheckTime();
      DATABASE_TOOLS_STATE.local = local;
      updateDatabaseToolsPanel();
      showScraperToast('Assignment check finished.', issues.length ? 'warning' : 'success', 3000);
      return;
    }

    if (action === 'integrity-check') {
      runDatabaseToolsDiagnostics();
      return;
    }

    if (action === 'recalculate-standings') {
      STANDINGS_FILTER_STATE.loadedOnce = false;
      loadStandingsCompetitionOptions(true);
      DATABASE_TOOLS_STATE.loading = false;
      DATABASE_TOOLS_STATE.message = 'Standings recalculation requested. Current backend has no dedicated rebuild endpoint, so the UI refreshed live standings from D1.';
      DATABASE_TOOLS_STATE.lastRun = formatBackendCheckTime();
      updateDatabaseToolsPanel();
      showScraperToast('Standings refreshed from backend.', 'success', 3000);
      return;
    }

    DATABASE_TOOLS_STATE.loading = false;
    DATABASE_TOOLS_STATE.message = 'Unknown maintenance action: ' + action;
    updateDatabaseToolsPanel();
  }


function runDatabaseToolsDiagnostics() {
    DATABASE_TOOLS_STATE.loading = true;
    DATABASE_TOOLS_STATE.message = 'Running backend verification...';
    DATABASE_TOOLS_STATE.local = getDatabaseToolsLocalSnapshot();
    DATABASE_TOOLS_STATE.lastRun = formatBackendCheckTime();
    updateDatabaseToolsPanel();

    var authHeader = { 'Authorization': 'Bearer ' + STATE_ADMIN_TOKEN_SESSION };

    var healthPromise = backendRequest(CF_ADMIN_HEALTH_ENDPOINT, {
      method: 'GET',
      headers: authHeader
    }, 8000).then(function(res) {
      return res.json().then(function(body) {
        DATABASE_TOOLS_STATE.health = Object.assign({ http_status: res.status }, body || {});
      });
    }).catch(function(err) {
      DATABASE_TOOLS_STATE.health = { ok: false, error: err && err.message ? err.message : String(err) };
    });

    var configPromise = backendRequest(CF_ADMIN_CONFIG_ENDPOINT, {
      method: 'GET',
      headers: authHeader
    }, 8000).then(function(res) {
      return res.json().then(function(body) {
        DATABASE_TOOLS_STATE.config = Object.assign({ http_status: res.status }, body || {});
      });
    }).catch(function(err) {
      DATABASE_TOOLS_STATE.config = { ok: false, error: err && err.message ? err.message : String(err) };
    });

    var archivePromise = backendRequest(CF_ADMIN_ARCHIVE_ENDPOINT, {
      method: 'GET',
      headers: authHeader
    }, 8000).then(function(res) {
      return res.json().then(function(body) {
        DATABASE_TOOLS_STATE.archive = Object.assign({ http_status: res.status }, body || {});
      });
    }).catch(function(err) {
      DATABASE_TOOLS_STATE.archive = { ok: false, error: err && err.message ? err.message : String(err) };
    });

    var resetPromise = backendRequest(CF_ADMIN_DEV_RESET_ENDPOINT, {
      method: 'POST',
      headers: Object.assign({}, authHeader, { 'Content-Type': 'application/json' }),
      body: JSON.stringify({ scope: '__verify_only__', confirm: 'VERIFY' })
    }, 8000).then(function(res) {
      return res.json().then(function(body) {
        // A 400 with supported_scopes means the endpoint exists and rejected the harmless verify payload.
        DATABASE_TOOLS_STATE.devReset = Object.assign({ http_status: res.status, endpoint_seen: res.status !== 404 }, body || {});
      });
    }).catch(function(err) {
      DATABASE_TOOLS_STATE.devReset = { ok: false, error: err && err.message ? err.message : String(err) };
    });

    Promise.all([healthPromise, configPromise, archivePromise, resetPromise]).then(function() {
      DATABASE_TOOLS_STATE.loading = false;
      var ready = getDatabaseToolsBackendReadiness();
      var score = [ready.health_ok, ready.config_ok, ready.archive_ok, ready.dev_reset_known, ready.authorized].filter(Boolean).length;
      DATABASE_TOOLS_STATE.message = 'Backend verification finished: ' + score + ' / 5 checks ready.';
      DATABASE_TOOLS_STATE.lastRun = formatBackendCheckTime();
      addActivityLog('Database Tools: backend verification finished (' + score + '/5).', score === 5 ? 'success' : 'warning');
      updateDatabaseToolsPanel();
    });
  }

  function updateDatabaseToolsPanel() {
    var panel = document.getElementById('nbf-database-tools-panel');
    if (!panel) return;

    var replacement = document.createElement('div');
    replacement.innerHTML = renderAdminDatabaseToolsBody();
    panel.parentNode.replaceChild(replacement.firstChild, panel);
    bindDatabaseToolsButtons();
  }

  function bindDatabaseToolsButtons() {
    var runBtn = document.getElementById('nbf-db-tools-run');
    if (runBtn) nbfBindOnce(runBtn, 'click', runDatabaseToolsDiagnostics, 'dbToolsRun');

    var refreshLocalBtn = document.getElementById('nbf-db-tools-refresh-local');
    if (refreshLocalBtn) nbfBindOnce(refreshLocalBtn, 'click', function() {
      DATABASE_TOOLS_STATE.local = getDatabaseToolsLocalSnapshot();
      DATABASE_TOOLS_STATE.message = 'Local snapshot refreshed.';
      DATABASE_TOOLS_STATE.lastRun = formatBackendCheckTime();
      updateDatabaseToolsPanel();
    }, 'dbToolsRefreshLocal');

    var maintenanceActions = [
      ['nbf-db-maint-refresh-cache', 'refresh-cache'],
      ['nbf-db-maint-recalculate-standings', 'recalculate-standings'],
      ['nbf-db-maint-verify-assignments', 'verify-assignments'],
      ['nbf-db-maint-integrity-check', 'integrity-check']
    ];

    maintenanceActions.forEach(function(pair) {
      var btn = document.getElementById(pair[0]);
      if (btn) nbfBindOnce(btn, 'click', function() { runDatabaseMaintenanceAction(pair[1]); }, 'dbMaint' + pair[1]);
    });

    var actions = [
      ['nbf-db-delete-standings', 'standings'],
      ['nbf-db-delete-assignments', 'assignments'],
      ['nbf-db-delete-events', 'events'],
      ['nbf-db-delete-seasons', 'seasons'],
      ['nbf-db-delete-competitions', 'competitions'],
      ['nbf-db-delete-uploaded-races', 'uploaded_races'],
      ['nbf-db-delete-factory', 'factory'],
      ['nbf-db-delete-queue', 'queue'],
      ['nbf-db-delete-submitted', 'submitted']
    ];

    actions.forEach(function(pair) {
      var btn = document.getElementById(pair[0]);
      if (btn) nbfBindOnce(btn, 'click', function() { runDatabaseDevReset(pair[1]); }, 'dbDevReset' + pair[1]);
    });
  }


  function renderAdminCompetitionManagerBody() {
    return renderComingSoonCard('Series Manager is coming later in the stable 6.5.x line. It will create named leagues/championships, select scoring modes, and assign standings to competitions.');
  }


  function renderDbToolsHealthChecklist(local, health, config) {
    var checks = [
      ['Userscript', 'v' + ENGINE_VERSION, true],
      ['Backend', health && health.ok === true ? 'online' : (BACKEND_STATUS.backend || 'not checked'), health && health.ok === true],
      ['D1 Database', health.database || config.database || BACKEND_STATUS.database || '—', String(health.database || config.database || BACKEND_STATUS.database || '').toLowerCase().indexOf('connected') !== -1],
      ['Admin Login', BACKEND_LOGIN_IDENTITY ? (BACKEND_LOGIN_IDENTITY.name || BACKEND_LOGIN_IDENTITY.torn_id || 'active') : 'locked', !!BACKEND_LOGIN_IDENTITY],
      ['Steward Token', STATE_STEWARD_TOKEN ? 'configured' : 'missing', !!STATE_STEWARD_TOKEN],
      ['Upload Queue', local.queue_failed ? (local.queue_failed + ' failed') : 'clean', !local.queue_failed],
      ['Championship Data', (local.seasons_total + ' season(s), ' + local.events_total + ' event(s)'), true],
      ['Race Assignments', local.standings_total ? 'standings available' : 'no assigned standings yet', true]
    ];

    return '<div style="display:grid; grid-template-columns:repeat(2,minmax(0,1fr)); gap:6px 12px; margin-top:8px;">' +
      checks.map(function(c) {
        return '<div style="display:flex; align-items:center; justify-content:space-between; gap:8px; padding:6px 8px; border:1px solid var(--nbf-bld); border-radius:7px; background:var(--nbf-main); font-size:11px;">' +
          '<span style="font-weight:700; color:var(--nbf-txt);">' + escapeHTML(c[0]) + '</span>' +
          '<span style="color:' + (c[2] ? '#16a34a' : '#f59e0b') + '; font-weight:700;">' + (c[2] ? '● ' : '● ') + escapeHTML(c[1]) + '</span>' +
        '</div>';
      }).join('') +
    '</div>';
  }

  function renderDbToolsDangerButton(id, label, count, enabled) {
    ...
    var status = renderDbToolsButtonStatus(scope);

    return '' +
      '<button id="' + escapeHTML(id) + '"' + disabled + ' style="' +
      'display:flex; align-items:center; justify-content:space-between; gap:10px;' +
      'padding:8px 10px; background:' + (enabled ? '#991b1b' : '#94a3b8') + '; color:#fff;' +
      'border:1px solid ' + (enabled ? '#7f1d1d' : '#94a3b8') + '; border-radius:6px;' +
      'font-size:11px; font-weight:700; cursor:' + (enabled ? 'pointer' : 'not-allowed') + ';' +
      'min-width:230px; max-width:280px; text-align:left; box-shadow:none;">' +
      '  <span style="display:flex; align-items:center; gap:6px; min-width:0; overflow:hidden;">' +
      '    <span>🗑</span><span style="overflow:hidden; text-overflow:ellipsis; white-space:nowrap;">' + escapeHTML(label) + '</span>' +
      '    <span style="opacity:.85;">(' + escapeHTML(count || 0) + ')</span>' +
      '  </span>' +
      '  <span style="flex:0 0 auto; padding:2px 6px; border-radius:999px; background:' + status.bg + '; color:' + status.fg + '; border:1px solid ' + status.border + '; font-size:9px; font-weight:800; line-height:1.2; text-transform:uppercase; letter-spacing:.02em;">' + escapeHTML(status.label) + '</span>' +
      '</button>';
  }

  function getDbToolsTypedConfirmation() {
    var input = document.getElementById('nbf-db-tools-confirm');
    return input ? String(input.value || '').trim().toUpperCase() : '';
  }


  function clearDatabaseToolsLocalFallback(scope) {
    var cleared = [];

    function removeLocal(key, label) {
      try {
    ...
    function setLocalJson(key, value, label) {
      try {
    ...
    if (scope === 'queue' || scope === 'factory') {
      setLocalJson(CONFIG_UPLOAD_QUEUE_KEY, [], 'local upload queue');
      setLocalJson(CONFIG_UPLOAD_HISTORY_KEY, [], 'local upload history');
    }

    if (scope === 'submitted' || scope === 'assignments' || scope === 'standings' || scope === 'factory') {
      try {
        // Submitted map is only a local duplicate guard. Clearing it does not delete server races.
        removeLocal(CONFIG_SUBMITTED_RACES_KEY, 'local submitted-race duplicate guard');
      } catch (err2) {}
    }

    if (scope === 'factory') {
      try { RUNTIME_MEMBERS = []; } catch (err3) {}
      try { RUNTIME_MAX_SKILL = 1; } catch (err4) {}
      removeLocal(CONFIG_CACHE_KEY, 'leaderboard cache');
      removeLocal(CONFIG_TIME_KEY, 'leaderboard cache timestamp');
      try {
        CHAMPIONSHIP_STATE.seasons = [];
        CHAMPIONSHIP_STATE.events = [];
        CHAMPIONSHIP_STATE.unassigned = [];
        CHAMPIONSHIP_STATE.standings = [];
      } catch (err5) {}
      try {
        COMPETITION_SETUP_STATE.competitions = [];
        COMPETITION_SETUP_STATE.participants = [];
      } catch (err6) {}
    }
  ...
  function runDatabaseDevResetLocalOnly(scope) {
    if (getDbToolsTypedConfirmation() !== 'DELETE') {
      showScraperToast('Type DELETE first.', 'warning', 2500);
      return;
    }

    var label = {
      queue: 'local queue/history',
      submitted: 'local submitted-race guard',
      factory: 'local development cache/state'
    }[scope] || scope;

    if (!window.confirm('Clear ' + label + ' locally in this browser? This does not delete remote D1 database records.')) return;

    var cleared = clearDatabaseToolsLocalFallback(scope);
    DATABASE_TOOLS_STATE.message = cleared.length
      ? 'Local cleanup completed: ' + cleared.join(', ') + '.'
      : 'Nothing local to clear for: ' + label + '.';
    DATABASE_TOOLS_STATE.lastRun = formatBackendCheckTime();

    addActivityLog('Database Tools local cleanup: ' + label + '.', 'warning');
    showScraperToast('Local cleanup completed.', 'success', 2500);
    updateDatabaseToolsPanel();
  }


  function runDatabaseDevReset(scope) {
    if (!BACKEND_LOGIN_IDENTITY || String(BACKEND_LOGIN_IDENTITY.role || '') !== 'super_admin') {
      showScraperToast('Development delete actions require super_admin.', 'error', 3000);
      return;
    }

    if (getDbToolsTypedConfirmation() !== 'DELETE') {
      showScraperToast('Type DELETE in Database Tools confirmation box first.', 'warning', 3500);
      return;
    }

    var label = {
      standings: 'global uploaded-race standings',
      assignments: 'race assignments',
      events: 'round/events',
      seasons: 'series',
      competitions: 'series records',
      uploaded_races: 'uploaded race results',
      queue: 'local queue/history',
      submitted: 'local submitted-race guard',
      factory: 'development test data'
    }[scope] || scope;

    if (scope === 'queue' || scope === 'submitted') {
      runDatabaseDevResetLocalOnly(scope);
      return;
    }

    if (!window.confirm('Delete ' + label + '? This requires Worker v1.0.13+ backend support and cannot be undone.')) return;

    DATABASE_TOOLS_STATE.loading = true;
    DATABASE_TOOLS_STATE.message = 'Requesting remote delete: ' + label + '...';
    updateDatabaseToolsPanel();

    backendRequest(CF_ADMIN_DEV_RESET_ENDPOINT, {
      method: 'POST',
      headers: {
        'Authorization': 'Bearer ' + STATE_ADMIN_TOKEN_SESSION,
        'Content-Type': 'application/json'
      },
      body: JSON.stringify({ scope: scope, confirm: 'DELETE' })
    }, 15000)
      .then(function(res) {
        return res.json().then(function(body) {
          if (!res.ok || !body.ok) {
      ...
      .then(function(body) {
        DATABASE_TOOLS_STATE.loading = false;
        DATABASE_TOOLS_STATE.message =
          'Remote delete completed: ' + label +
          '. Deleted rows: ' + (body.deleted_total !== undefined ? body.deleted_total : 'unknown') + '.';
        DATABASE_TOOLS_STATE.lastRun = formatBackendCheckTime();
        DATABASE_TOOLS_STATE.local = getDatabaseToolsLocalSnapshot();

        addActivityLog('Database Tools remote delete completed: ' + label + '.', 'warning');
        showScraperToast('Deleted: ' + label + '.', 'success', 3000);

        // Reset stale frontend state after destructive backend actions.
        if (scope === 'uploaded_races' || scope === 'standings' || scope === 'factory') {
          clearDatabaseToolsLocalFallback('submitted');
          STANDINGS_FILTER_STATE.loadedOnce = false;
        }
      ...
      .catch(function(err) {
        ...
        var missingEndpoint = err && (
          err.httpStatus === 404 ||
          String(err.message || '').indexOf('404') !== -1 ||
        ...
        var authProblem = err && (
          ...
          String(err.message || '').toLowerCase().indexOf('super_admin') !== -1
        );

        if (missingEndpoint) {
          DATABASE_TOOLS_STATE.message =
            'Remote delete endpoint is not installed/deployed yet. Deploy worker.js v1.0.13 first.';
        } else if (authProblem) {
          DATABASE_TOOLS_STATE.message =
            'Remote delete blocked by backend authorization. Re-login as super_admin and retry.';
        } else {
          DATABASE_TOOLS_STATE.message =
            'Remote delete failed: ' + (err && err.message ? err.message : String(err));
        }

        addActivityLog('Database Tools remote delete failed for: ' + label + '.', 'warning');
        showScraperToast(DATABASE_TOOLS_STATE.message, 'error', 5000);
        updateDatabaseToolsPanel();
      });
  }
  ...
  function renderAdminDatabaseToolsBody() {
    if (!BACKEND_LOGIN_IDENTITY || !canManageBackendStewards()) {
      return '<div id="nbf-database-tools-panel" style="font-size:12px; color:var(--nbf-mut); padding:12px; border:1px solid var(--nbf-bld); border-radius:8px; background:var(--nbf-main);">Database Tools are admin-only. Log in as admin or super_admin to view diagnostics.</div>';
    }

    if (!DATABASE_TOOLS_STATE.local) DATABASE_TOOLS_STATE.local = getDatabaseToolsLocalSnapshot();

    var local = DATABASE_TOOLS_STATE.local || getDatabaseToolsLocalSnapshot();
    var health = DATABASE_TOOLS_STATE.health || {};
    var config = DATABASE_TOOLS_STATE.config || {};
    var isSuperAdmin = BACKEND_LOGIN_IDENTITY && String(BACKEND_LOGIN_IDENTITY.role || '') === 'super_admin';

    var healthLabel = health.ok === true ? 'Online' : (health.ok === false ? 'Check failed' : 'Not run');
    var dbLabel = health.database || config.database || BACKEND_STATUS.database || '—';
    var configLabel = config.ok === true ? 'OK' : (config.ok === false ? 'Check failed' : 'Not run');

    var msg = DATABASE_TOOLS_STATE.message
      ? '<div style="margin:10px 0; padding:8px 10px; border:1px solid var(--nbf-bld); border-radius:7px; background:var(--nbf-main); font-size:12px; color:var(--nbf-mut);">' + escapeHTML(DATABASE_TOOLS_STATE.message) + '</div>'
      : '';

    var healthError = health.error ? '<div style="font-size:11px; color:#dc2626; margin-top:5px;">Health error: ' + escapeHTML(health.error) + '</div>' : '';
    var configError = config.error ? '<div style="font-size:11px; color:#dc2626; margin-top:5px;">Config error: ' + escapeHTML(config.error) + '</div>' : '';

    return '' +
      '<div id="nbf-database-tools-panel">' +
      '  <div style="display:flex; align-items:center; justify-content:space-between; gap:12px; border-bottom:1px solid var(--nbf-bld); padding-bottom:8px; margin-bottom:12px;">' +
      '    <div>' +
      '      <h3 style="margin:0; font-size:14px; color:var(--nbf-txt);">Database Tools</h3>' +
      '      <div style="font-size:11px; color:var(--nbf-mut); margin-top:3px;">Diagnostics, health checklist, maintenance preview, and guarded development cleanup.</div>' +
      '    </div>' +
      '    <div style="display:flex; gap:8px; flex-wrap:wrap;">' +
      '      <button id="nbf-db-tools-run" style="padding:7px 11px; background:#6366f1; color:#fff; border:none; border-radius:6px; font-size:11px; font-weight:700; cursor:pointer;">' + (DATABASE_TOOLS_STATE.loading ? 'Running...' : 'Run Diagnostics') + '</button>' +
      '      <button id="nbf-db-tools-refresh-local" style="padding:7px 11px; background:var(--nbf-field-bg); color:var(--nbf-txt); border:1px solid var(--nbf-btn-border); border-radius:6px; font-size:11px; cursor:pointer;">Refresh Snapshot</button>' +
      '    </div>' +
      '  </div>' +
      msg +

      '  <h4 style="margin:0 0 6px 0; font-size:12px; color:var(--nbf-txt);">Database Diagnostics</h4>' +
      '  <div style="display:grid; grid-template-columns:repeat(4,minmax(0,1fr)); gap:8px; margin-bottom:10px;">' +
      renderDbToolCard('Admin Health', healthLabel, 'Worker admin endpoint') +
      renderDbToolCard('Database', dbLabel, 'D1 binding status') +
      renderDbToolCard('Config Check', configLabel, 'Bindings / secrets') +
      renderDbToolCard('Last Run', DATABASE_TOOLS_STATE.lastRun || '—', 'Local time') +
      '  </div>' +
      healthError + configError +
      renderDbToolsHealthChecklist(local, health, config) +

      '  <h4 style="margin:16px 0 6px 0; font-size:12px; color:var(--nbf-txt);">Database Snapshot</h4>' +
      '  <div style="display:grid; grid-template-columns:repeat(4,minmax(0,1fr)); gap:8px; margin-bottom:10px;">' +
      renderDbToolCard('Series', local.competitions_total, 'competition records') +
      renderDbToolCard('Seasons', local.seasons_total, 'championship seasons') +
      renderDbToolCard('Events', local.events_total, 'championship events') +
      renderDbToolCard('Standings Rows', local.standings_total, 'assigned result standings') +
      '  </div>' +
      '  <div style="display:grid; grid-template-columns:repeat(4,minmax(0,1fr)); gap:8px; margin-bottom:10px;">' +
      renderDbToolCard('Unassigned Races', local.unassigned_total, 'uploaded but not linked') +
      renderDbToolCard('Participants', local.participants_total, 'loaded participant rows') +
      renderDbToolCard('Queue Items', local.queue_total, 'local upload queue') +
      renderDbToolCard('Activity Entries', local.activity_total, 'local steward log') +
      '  </div>' +

      '  <h4 style="margin:16px 0 6px 0; font-size:12px; color:var(--nbf-txt);">Upload Queue Snapshot</h4>' +
      '  <div style="display:grid; grid-template-columns:repeat(4,minmax(0,1fr)); gap:8px; margin-bottom:10px;">' +
      renderDbToolCard('Pending', local.queue_pending, 'not terminal') +
      renderDbToolCard('Completed', local.queue_completed, 'still stored locally') +
      renderDbToolCard('Failed', local.queue_failed, 'failed_permanent') +
      renderDbToolCard('Submitted Guard', local.submitted_total, 'local duplicate map') +
      '  </div>' +
      '  <h4 style="margin:12px 0 6px 0; font-size:12px; color:var(--nbf-txt);">Latest Queue Items</h4>' +
      renderDbToolsMiniRows(local.latest_queue, 'No local queue items.') +
      '  <h4 style="margin:12px 0 6px 0; font-size:12px; color:var(--nbf-txt);">Latest Upload History</h4>' +
      renderDbToolsMiniRows(local.latest_history, 'No local upload history.') +

      '  <h4 style="margin:16px 0 6px 0; font-size:12px; color:var(--nbf-txt);">Database Maintenance</h4>' +
      '  <div style="display:flex; gap:8px; flex-wrap:wrap; margin-bottom:10px;">' +
      renderDbToolsLockedAction('Refresh Cache', 'Requires backend maintenance endpoint') +
      renderDbToolsLockedAction('Recalculate Standings', 'Requires backend maintenance endpoint') +
      renderDbToolsLockedAction('Verify Assignments', 'Requires backend maintenance endpoint') +
      renderDbToolsLockedAction('Integrity Check', 'Requires backend maintenance endpoint') +
      '  </div>' +

      '  <h4 style="margin:16px 0 6px 0; font-size:12px; color:#dc2626;">Development Utilities</h4>' +
      '  <div style="font-size:11px; color:var(--nbf-mut); line-height:1.45; margin-bottom:8px;">Use only for test cleanup. Requires <strong>super_admin</strong>, typed confirmation, backend support, and audit logging. Type <strong>DELETE</strong> to enable requests.</div>' +
      '  <input id="nbf-db-tools-confirm" placeholder="Type DELETE to enable development delete requests" style="width:100%; box-sizing:border-box; margin-bottom:8px; padding:7px 9px; border:1px solid var(--nbf-btn-border); border-radius:6px; background:var(--nbf-field-bg); color:var(--nbf-txt); font-size:12px;">' +
      '  <div style="display:flex; gap:8px; flex-wrap:wrap; margin-bottom:10px;">' +
      renderDbToolsDangerButton('nbf-db-delete-standings', 'Delete Standings', local.standings_total, isSuperAdmin) +
      renderDbToolsDangerButton('nbf-db-delete-assignments', 'Delete Assignments', local.standings_total, isSuperAdmin) +
      renderDbToolsDangerButton('nbf-db-delete-events', 'Delete Events', local.events_total, isSuperAdmin) +
      renderDbToolsDangerButton('nbf-db-delete-seasons', 'Delete Seasons', local.seasons_total, isSuperAdmin) +
      renderDbToolsDangerButton('nbf-db-delete-competitions', 'Delete Series', local.competitions_total, isSuperAdmin) +
      renderDbToolsDangerButton('nbf-db-delete-uploaded-races', 'Delete Uploaded Race Results', local.submitted_total || local.history_total || 0, isSuperAdmin) +
      renderDbToolsDangerButton('nbf-db-delete-factory', 'Factory Reset Tests', (local.seasons_total + local.events_total + local.competitions_total), isSuperAdmin) +
      renderDbToolsDangerButton('nbf-db-delete-queue', 'Clear Local Queue', local.queue_total + local.history_total, true) +
      renderDbToolsDangerButton('nbf-db-delete-submitted', 'Clear Local Submitted Guard', local.submitted_total, true) +
      '  </div>' +
      '  <div style="font-size:11px; color:var(--nbf-mut); line-height:1.45; border-top:1px solid var(--nbf-bld); padding-top:10px;">No raw SQL is exposed in the userscript. Run Backend Verification first: buttons show whether they are local-only, backend-ready, or blocked by permissions/deployment.</div>' +
      '</div>';
  }

  function renderAdminArchiveBody() {
    return '' +
      '<div style="border:1px solid #16a34a55; background:#f0fdf4; border-radius:10px; padding:12px 14px; color:#166534; font-size:12px; line-height:1.5;">' +
      '  <div style="font-weight:800; margin-bottom:5px;">Archive backend support expected in Worker v1.0.13+</div>' +
      '  <div>Use the check button after deploying the updated Worker. GET /admin/archive should return installed:true.</div>' +
      '  <div style="margin-top:8px; font-family:monospace; font-size:11px;">Endpoint: /admin/archive</div>' +
      '</div>' +
      '<div style="margin-top:10px; display:flex; gap:8px; flex-wrap:wrap;">' +
      '  <button id="nbf-archive-check-btn" style="padding:7px 12px; background:var(--nbf-field-bg); color:var(--nbf-txt); border:1px solid var(--nbf-btn-border); border-radius:6px; font-size:12px; cursor:pointer;">Check Archive Endpoint</button>' +
      '  <button disabled style="padding:7px 12px; background:#94a3b8; color:#fff; border:none; border-radius:6px; font-size:12px; cursor:not-allowed;">Archive Series UI Next</button>' +
      '</div>';
  }

  function renderStewardUploadStatusBody() {
    var html = renderIngestionStatusMarkup('✅ ' + CF_INGEST_ENDPOINT);

    try {
      if (typeof renderUploadCompletionSummary === 'function') {
        html += renderUploadCompletionSummary();
      }
    } catch (err) {
      console.error('[NB Racing] Upload summary render failed:', err);
      html += '<div style="margin-top:10px; color:#f59e0b; font-size:12px;">Summary render error — check console.</div>';
    }
  ...
  function renderStewardChecklistBody() {
    return '<h3 style="margin:0 0 8px 0; font-size:13px; color:var(--nbf-txt);">Upload Readiness</h3>' +
      '<div style="font-size:11px; color:var(--nbf-mut); margin-bottom:8px;">Full system health is now in Database Tools. This checklist only shows upload gates for the current race page.</div>' +
      renderGateChecklistMarkup();
  }
  ...
  function createChampionshipSeason() {
    ...
    if (!name) {
      showScraperToast('Series name required.', 'error', 2500);
      return;
    }

    if (!year || year < 2020 || year > 2100) {
      showScraperToast('Valid season year required.', 'error', 2500);
      return;
    }
    ...
    var seasonConfig = {
      ...
      status: 'draft',
      created_from: 'userscript',
      created_by_version: ENGINE_VERSION
    };
    ...
    var payload = {
      ...
      // Compatibility fields for stricter/newer backend schemas.
      config_json: JSON.stringify(seasonConfig),
      config: seasonConfig
    };

    CHAMPIONSHIP_STATE.loading = true;
    CHAMPIONSHIP_STATE.message = 'Creating season...';
    renderStewardPanelContent();

    championshipRequest(CF_CHAMPIONSHIP_SEASONS_ENDPOINT, {
      method: 'POST',
      body: JSON.stringify(payload)
    })
      .then(function(r) {
        return r.json().then(function(b) {
          if (!r.ok || !b.ok) throw new Error(b && b.error ? b.error : ('HTTP ' + r.status));
          return b;
        });
      })
      .then(function() {
        CHAMPIONSHIP_STATE.loading = false;
        CHAMPIONSHIP_STATE.message = 'Season created: ' + name + '.';
        showScraperToast('Season created.', 'success', 2200);
        fetchChampionshipData();
      })
      .catch(function(e) {
        CHAMPIONSHIP_STATE.loading = false;
        CHAMPIONSHIP_STATE.message = 'Season create failed: ' + (e && e.message ? e.message : String(e));
        showScraperToast(CHAMPIONSHIP_STATE.message, 'error', 4500);
        renderStewardPanelContent();
      });
  }
  ...
  function createChampionshipEvent() {
    ...
    if (!seasonId) {
      showScraperToast('Create or select a season first.', 'error', 3000);
      return;
    }

    if (!name) {
      showScraperToast('Round/Event name required.', 'error', 2500);
      return;
    }
    ...
    var payload = {
      ...
      // Compatibility aliases for backend handlers that use different naming.
      seasonId: seasonId,
      round: round,

      // Compatibility fields for stricter/newer backend schemas.
      config_json: JSON.stringify(eventConfig),
      config: eventConfig
    };

    CHAMPIONSHIP_STATE.loading = true;
    CHAMPIONSHIP_STATE.message = 'Creating event...';
    renderStewardPanelContent();

    championshipRequest(CF_CHAMPIONSHIP_EVENTS_ENDPOINT, {
      method: 'POST',
      body: JSON.stringify(payload)
    })
      .then(function(r) {
      ...
      .then(function() {
        CHAMPIONSHIP_STATE.loading = false;
        CHAMPIONSHIP_STATE.message = 'Event created: ' + name + '.';
        showScraperToast('Event created.', 'success', 2200);
        fetchChampionshipData();
      })
      .catch(function(e) {
        CHAMPIONSHIP_STATE.loading = false;
        CHAMPIONSHIP_STATE.message = 'Event create failed: ' + (e && e.message ? e.message : String(e));
        showScraperToast(CHAMPIONSHIP_STATE.message, 'error', 4500);
        renderStewardPanelContent();
      });
  }
  ...
  function assignRaceToChampionshipEvent() {
    ...
    championshipRequest(CF_CHAMPIONSHIP_ASSIGN_ENDPOINT,{method:'POST',body:JSON.stringify({race_id:raceId,event_id:eventId})})
      ...
      .then(function(){showScraperToast('Race assigned.','success',2200);refreshLeagueSetupAndStandings();})
      .catch(function(e){showScraperToast('Assign failed: '+(e.message||e),'error',3500);});
  }


  function competitionRequest(url, options, timeoutMs) {
    options = options || {};
    options.headers = options.headers || {};
    options.headers['Authorization'] = 'Bearer ' + STATE_ADMIN_TOKEN_SESSION;
    if (options.body && !options.headers['Content-Type']) options.headers['Content-Type'] = 'application/json';
    return backendRequest(url, options, timeoutMs || 9000);
  }

  function fetchCompetitionSetupData() {
    if (!isAdminUnlocked()) return;
    COMPETITION_SETUP_STATE.loading = true;

    return Promise.all([
      competitionRequest(CF_COMPETITION_TEMPLATES_ENDPOINT, { method: 'GET' }),
      competitionRequest(CF_COMPETITIONS_ENDPOINT, { method: 'GET' })
    ]).then(function(rs) {
      ...
      COMPETITION_SETUP_STATE.templates = payloads[0].templates || {};
      COMPETITION_SETUP_STATE.competitions = payloads[1].competitions || [];
      COMPETITION_SETUP_STATE.loading = false;
      COMPETITION_SETUP_STATE.message = 'Competition setup loaded.';
      renderStewardPanelContent();
    }).catch(function(err) {
      COMPETITION_SETUP_STATE.loading = false;
      COMPETITION_SETUP_STATE.message = 'Competition setup load failed: ' + (err && err.message ? err.message : String(err));
      showScraperToast(COMPETITION_SETUP_STATE.message, 'error', 3500);
      renderStewardPanelContent();
    });
  }

  function createCompetitionFromTemplate() {
    var nameEl = document.getElementById('nbf-comp-name');
    var typeEl = document.getElementById('nbf-comp-template');
    var seasonEl = document.getElementById('nbf-comp-season-id');

    var name = sanitizePlainText(nameEl ? nameEl.value : '', 100);
    var templateType = sanitizePlainText(typeEl ? typeEl.value : 'CUSTOM', 20);
    var seasonId = Number(seasonEl ? seasonEl.value : 0);
    ...
    var templates = COMPETITION_SETUP_STATE.templates || {};
    var template = templates[templateType] || templates[templateType.toUpperCase()] || templates.CUSTOM || {};
    var defaultConfig = template.default_config || template.config || {
      phases: ['registration'],
      grouping: {},
      scoring: { type: 'standard' },
      schedule: {},
      advancement: {}
    };
    ...
    var payload = {
      ...
      // Required by the D1 competitions schema.
      // Some backend versions expect config_json as a string, newer ones may accept config.
    ...
    COMPETITION_SETUP_STATE.loading = true;
    COMPETITION_SETUP_STATE.message = 'Creating Series template...';
    renderStewardPanelContent();

    competitionRequest(CF_COMPETITIONS_ENDPOINT, {
      method: 'POST',
      body: JSON.stringify(payload)
    }).then(function(res) {
      return res.json().then(function(body) {
        if (!res.ok || !body.ok) throw new Error(body && body.error ? body.error : 'HTTP ' + res.status);
        return body;
      });
    }).then(function(body) {
      COMPETITION_SETUP_STATE.loading = false;
      COMPETITION_SETUP_STATE.message = 'Series template created: ' + name + '.';
      showScraperToast('Series template created.', 'success', 2200);
      fetchCompetitionSetupData();
      loadStandingsCompetitionOptions(true);
    }).catch(function(err) {
      COMPETITION_SETUP_STATE.loading = false;
      COMPETITION_SETUP_STATE.message = 'Competition create failed: ' + (err && err.message ? err.message : String(err));
      showScraperToast(COMPETITION_SETUP_STATE.message, 'error', 4500);
      renderStewardPanelContent();
    });
  }

  function importCompetitionParticipants() {
    var compEl = document.getElementById('nbf-participant-competition');
    var textEl = document.getElementById('nbf-participant-import-text');

    var competitionId = Number(compEl ? compEl.value : 0);
    var raw = textEl ? textEl.value.trim() : '';

    if (!competitionId || !raw) {
      showScraperToast('Competition and participant rows required.', 'error', 2500);
      return;
    }

    competitionRequest(CF_COMPETITION_IMPORT_ENDPOINT, {
      method: 'POST',
      body: JSON.stringify({
      ...
      showScraperToast('Participants imported: ' + body.imported + ', skipped: ' + body.skipped, 'success', 3500);
      COMPETITION_SETUP_STATE.message = 'Imported ' + body.imported + ' participant(s), skipped ' + body.skipped + '.';
      fetchCompetitionParticipants(competitionId);
      fetchCompetitionSetupData();
    }).catch(function(err) {
      showScraperToast('Participant import failed: ' + (err && err.message ? err.message : String(err)), 'error', 4000);
    });
  }

  function fetchCompetitionParticipants(competitionId) {
    if (!competitionId) return;

    competitionRequest(CF_COMPETITION_PARTICIPANTS_ENDPOINT + '?competition_id=' + encodeURIComponent(competitionId), { method: 'GET' })
      .then(function(res) {
        return res.json().then(function(body) {
          if (!res.ok || !body.ok) throw new Error(body && body.error ? body.error : 'HTTP ' + res.status);
          return body;
      ...
      .then(function(body) {
        COMPETITION_SETUP_STATE.participants = body.participants || [];
        renderStewardPanelContent();
      })
      .catch(function(err) {
        showScraperToast('Participant list failed: ' + (err && err.message ? err.message : String(err)), 'error', 3500);
      });
  }

  function renderCompetitionTemplateHint(type) {
    var templates = COMPETITION_SETUP_STATE.templates || {};
    var t = templates[type] || templates.CUSTOM || {};
    var desc = t.description || '';
    var cfg = t.default_config ? JSON.stringify(t.default_config, null, 2) : '{}';

    return '' +
      '<div style="border:1px solid var(--nbf-bld); border-radius:7px; padding:10px; background:var(--nbf-main); margin-top:8px;">' +
      '  <div style="font-size:12px; font-weight:700; color:var(--nbf-txt);">' + escapeHTML(t.name || type) + '</div>' +
      '  <div style="font-size:11px; color:var(--nbf-mut); line-height:1.45; margin-top:4px;">' + escapeHTML(desc) + '</div>' +
      '  <pre style="white-space:pre-wrap; font-size:10px; color:var(--nbf-mut); background:var(--nbf-alt); padding:8px; border-radius:6px; max-height:160px; overflow:auto;">' + escapeHTML(cfg) + '</pre>' +
      '</div>';
  }

  function renderCompetitionSetupBody() {
    if (!isAdminUnlocked()) {
      return '<div style="font-size:12px; color:var(--nbf-mut); padding:8px 0;">Login as Super Admin/Admin/Chief to manage competition setup.</div>';
    }

    var templates = COMPETITION_SETUP_STATE.templates || {};
    var competitions = COMPETITION_SETUP_STATE.competitions || [];
    var participants = COMPETITION_SETUP_STATE.participants || [];
    var templateKeys = Object.keys(templates).length ? Object.keys(templates) : ['JLT', 'URT', 'CUSTOM'];

    var templateOptions = templateKeys.map(function(k) {
      return '<option value="' + escapeHTML(k) + '">' + escapeHTML((templates[k] && templates[k].name) || k) + '</option>';
    }).join('');

    var compOptions = competitions.map(function(c) {
      return '<option value="' + escapeHTML(c.id) + '">' + escapeHTML('#' + c.id + ' · ' + c.name + ' · ' + c.template_type) + '</option>';
    }).join('');

    var seasonOptions = (CHAMPIONSHIP_STATE.seasons || []).map(function(s) {
      return '<option value="' + escapeHTML(s.id) + '">' + escapeHTML(s.name + ' (' + s.year + ')') + '</option>';
    }).join('');

    var html = '<div id="nbf-competition-setup-panel">' +
      '<div style="display:flex; justify-content:space-between; align-items:center; gap:8px; margin-bottom:10px; flex-wrap:wrap;">' +
      '<div style="font-size:12px; color:var(--nbf-mut);">' + escapeHTML(COMPETITION_SETUP_STATE.message || 'Template and participant database foundation.') + '</div>' +
      '<button id="nbf-comp-refresh-btn" style="padding:6px 10px; background:#6366f1; color:#fff; border:none; border-radius:6px; font-size:11px; font-weight:600; cursor:pointer;">Refresh</button>' +
      '</div>' +

      '<div style="display:grid; grid-template-columns:repeat(4,minmax(0,1fr)); gap:8px; margin-bottom:12px;">' +
      renderStatModule('Templates', templateKeys.length) +
      renderStatModule('Series', competitions.length) +
      renderStatModule('Participants Loaded', participants.length) +
      renderStatModule('Status', COMPETITION_SETUP_STATE.loading ? 'Loading' : 'Ready') +
      '</div>' +

      '<div style="display:grid; grid-template-columns:1fr 1fr; gap:12px; margin-bottom:12px;">' +
      '<div style="border:1px solid var(--nbf-bld); border-radius:7px; padding:10px; background:var(--nbf-main);">' +
      '<h4 style="margin:0 0 8px 0; color:var(--nbf-txt); font-size:12px;">Create Series</h4>' +
      '<input id="nbf-comp-name" placeholder="Series name, e.g. Nuclear Family JLT" style="width:100%; box-sizing:border-box; margin-bottom:6px; padding:7px 9px; border:1px solid var(--nbf-btn-border); border-radius:6px; background:var(--nbf-field-bg); color:var(--nbf-txt); font-size:12px;">' +
      '<select id="nbf-comp-template" style="width:100%; box-sizing:border-box; margin-bottom:6px; padding:7px 9px; border:1px solid var(--nbf-btn-border); border-radius:6px; background:var(--nbf-field-bg); color:var(--nbf-txt); font-size:12px;">' + templateOptions + '</select>' +
      '<select id="nbf-comp-season-id" style="width:100%; box-sizing:border-box; margin-bottom:8px; padding:7px 9px; border:1px solid var(--nbf-btn-border); border-radius:6px; background:var(--nbf-field-bg); color:var(--nbf-txt); font-size:12px;"><option value="">No linked season yet</option>' + seasonOptions + '</select>' +
      '<button id="nbf-comp-create-btn" style="padding:7px 12px; background:#16a34a; color:#fff; border:none; border-radius:6px; font-size:12px; font-weight:600; cursor:pointer;">Create Series</button>' +
      renderCompetitionTemplateHint('JLT') +
      '</div>' +

      '<div style="border:1px solid var(--nbf-bld); border-radius:7px; padding:10px; background:var(--nbf-main);">' +
      '<h4 style="margin:0 0 8px 0; color:var(--nbf-txt); font-size:12px;">Participant Import</h4>' +
      '<select id="nbf-participant-competition" style="width:100%; box-sizing:border-box; margin-bottom:6px; padding:7px 9px; border:1px solid var(--nbf-btn-border); border-radius:6px; background:var(--nbf-field-bg); color:var(--nbf-txt); font-size:12px;">' + compOptions + '</select>' +
      '<textarea id="nbf-participant-import-text" placeholder="Paste CSV or TSV rows here. Header examples: torn_id,display_name,faction,racing_skill,racer_class,car_class" style="width:100%; box-sizing:border-box; min-height:160px; margin-bottom:8px; padding:8px 9px; border:1px solid var(--nbf-btn-border); border-radius:6px; background:var(--nbf-field-bg); color:var(--nbf-txt); font-size:11px; font-family:monospace;"></textarea>' +
      '<div style="display:flex; gap:8px; flex-wrap:wrap;">' +
      '<button id="nbf-participant-import-btn" style="padding:7px 12px; background:#16a34a; color:#fff; border:none; border-radius:6px; font-size:12px; font-weight:600; cursor:pointer;">Import Participants</button>' +
      '<button id="nbf-participant-load-btn" style="padding:7px 12px; background:#6366f1; color:#fff; border:none; border-radius:6px; font-size:12px; font-weight:600; cursor:pointer;">Load Participants</button>' +
      '</div>' +
      '</div>' +
      '</div>';

    html += '<h4 style="margin:8px 0; font-size:12px; color:var(--nbf-txt);">Series</h4>';
    if (!competitions.length) {
      html += '<div style="font-size:12px; color:var(--nbf-mut); padding:8px 0;">No competitions created yet.</div>';
    } else {
      competitions.slice(0, 12).forEach(function(c) {
        html += '<div style="display:grid; grid-template-columns:54px 1fr 110px 90px 80px; gap:8px; padding:6px 0; border-bottom:1px solid var(--nbf-bld); font-size:11px;">' +
          '<div>#' + escapeHTML(c.id) + '</div>' +
          '<div style="font-weight:700; color:var(--nbf-txt);">' + escapeHTML(c.name || '') + '</div>' +
          '<div>' + escapeHTML(c.template_type || '') + '</div>' +
          '<div>' + escapeHTML(c.status || '') + '</div>' +
          '<div>' + escapeHTML(c.season_id || '—') + '</div>' +
          '</div>';
      });
    }

    html += '<h4 style="margin:12px 0 8px 0; font-size:12px; color:var(--nbf-txt);">Loaded Participants</h4>';
    if (!participants.length) {
      html += '<div style="font-size:12px; color:var(--nbf-mut); padding:8px 0;">No participant list loaded.</div>';
    } else {
      participants.slice(0, 20).forEach(function(p) {
        html += '<div style="display:grid; grid-template-columns:90px 1fr 100px 70px 70px; gap:8px; padding:5px 0; border-bottom:1px solid var(--nbf-bld); font-size:11px;">' +
          '<div style="font-family:monospace;">' + escapeHTML(p.torn_id || '—') + '</div>' +
          '<div style="font-weight:700; color:var(--nbf-txt);">' + escapeHTML(p.display_name || '') + '</div>' +
          '<div>' + escapeHTML(p.faction || '—') + '</div>' +
          '<div>' + escapeHTML(p.racing_skill || '—') + '</div>' +
          '<div>' + escapeHTML(p.racer_class || '—') + '</div>' +
          '</div>';
      });
    }
  ...
  function bindCompetitionSetupButtons() {
    var refreshBtn = document.getElementById('nbf-comp-refresh-btn');
    if (refreshBtn) refreshBtn.addEventListener('click', fetchCompetitionSetupData);

    var createBtn = document.getElementById('nbf-comp-create-btn');
    if (createBtn) createBtn.addEventListener('click', createCompetitionFromTemplate);

    var importBtn = document.getElementById('nbf-participant-import-btn');
    if (importBtn) importBtn.addEventListener('click', importCompetitionParticipants);

    var loadBtn = document.getElementById('nbf-participant-load-btn');
    if (loadBtn) loadBtn.addEventListener('click', function() {
      var compEl = document.getElementById('nbf-participant-competition');
      fetchCompetitionParticipants(Number(compEl ? compEl.value : 0));
    });
  }
  ...
  function renderChampionshipEngineBody() {
    ...
    var seasons = CHAMPIONSHIP_STATE.seasons || [];
    var events = CHAMPIONSHIP_STATE.events || [];
    var unassigned = CHAMPIONSHIP_STATE.unassigned || [];
    var standings = CHAMPIONSHIP_STATE.standings || [];

    var seasonOptions = seasons.length
      ? seasons.map(function(s) { return '<option value="' + escapeHTML(s.id) + '">' + escapeHTML(s.name + ' (' + s.year + ')') + '</option>'; }).join('')
      : '<option value="">Create a series first</option>';

    var eventOptions = events.length
      ? events.map(function(e) { return '<option value="' + escapeHTML(e.id) + '">' + escapeHTML((e.season_name || 'Season') + ' · R' + (e.round_number || '?') + ' · ' + e.name) + '</option>'; }).join('')
      : '<option value="">Create a round/event first</option>';

    var raceOptions = unassigned.length
      ? unassigned.map(function(r) { return '<option value="' + escapeHTML(r.race_id) + '">' + escapeHTML(r.race_id + ' · ' + (r.result_count || 0) + ' results') + '</option>'; }).join('')
      : '<option value="">No unassigned races</option>';
    ...
    var html = '<div id="nbf-championship-panel">' +
      ...
      '<span style="font-size:12px;color:var(--nbf-mut);">' + escapeHTML(CHAMPIONSHIP_STATE.message || 'Simple flow: create series → create round/event → assign uploaded race.') + '</span>' +
      '<button id="nbf-champ-refresh-btn" style="padding:6px 10px;background:#6366f1;color:#fff;border:none;border-radius:6px;font-size:11px;font-weight:600;cursor:pointer;">Refresh</button>' +
      '</div>' +
      '<div style="display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:8px;margin-bottom:12px;">' +
      renderStatModule('Seasons', seasons.length) + renderStatModule('Events', events.length) + renderStatModule('Unassigned', unassigned.length) + renderStatModule('Ranked Drivers', standings.length) +
      '</div>' +
      '<div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:12px;">' +
      '<div style="border:1px solid var(--nbf-bld);border-radius:7px;padding:10px;background:var(--nbf-main);"><h4 style="margin:0 0 4px;color:var(--nbf-txt);font-size:12px;">1. Create Series</h4><div style="font-size:11px;color:var(--nbf-mut);margin-bottom:8px;">Container for events and assigned races.</div><input id="nbf-champ-season-name" placeholder="Series name" style="width:100%;box-sizing:border-box;margin-bottom:6px;padding:7px 9px;border:1px solid var(--nbf-btn-border);border-radius:6px;background:var(--nbf-field-bg);color:var(--nbf-txt);font-size:12px;"><input id="nbf-champ-season-year" type="number" value="' + new Date().getFullYear() + '" style="width:100%;box-sizing:border-box;margin-bottom:8px;padding:7px 9px;border:1px solid var(--nbf-btn-border);border-radius:6px;background:var(--nbf-field-bg);color:var(--nbf-txt);font-size:12px;"><button id="nbf-champ-create-season-btn" style="padding:7px 12px;background:#16a34a;color:#fff;border:none;border-radius:6px;font-size:12px;font-weight:600;cursor:pointer;">Create Series</button></div>' +
      '<div style="border:1px solid var(--nbf-bld);border-radius:7px;padding:10px;background:var(--nbf-main);"><h4 style="margin:0 0 4px;color:var(--nbf-txt);font-size:12px;">2. Create Round/Event</h4><div style="font-size:11px;color:var(--nbf-mut);margin-bottom:8px;">Round/Event inside a series.</div><select id="nbf-champ-event-season" style="width:100%;box-sizing:border-box;margin-bottom:6px;padding:7px 9px;border:1px solid var(--nbf-btn-border);border-radius:6px;background:var(--nbf-field-bg);color:var(--nbf-txt);font-size:12px;">' + seasonOptions + '</select>' + (!seasons.length ? '<div style="font-size:11px;color:#b45309;margin-bottom:6px;">Create a series first.</div>' : '') + '<input id="nbf-champ-event-name" placeholder="Round/Event name" style="width:100%;box-sizing:border-box;margin-bottom:6px;padding:7px 9px;border:1px solid var(--nbf-btn-border);border-radius:6px;background:var(--nbf-field-bg);color:var(--nbf-txt);font-size:12px;"><input id="nbf-champ-event-track" placeholder="Track optional" style="width:100%;box-sizing:border-box;margin-bottom:6px;padding:7px 9px;border:1px solid var(--nbf-btn-border);border-radius:6px;background:var(--nbf-field-bg);color:var(--nbf-txt);font-size:12px;"><input id="nbf-champ-event-round" type="number" value="' + (events.length + 1) + '" style="width:100%;box-sizing:border-box;margin-bottom:8px;padding:7px 9px;border:1px solid var(--nbf-btn-border);border-radius:6px;background:var(--nbf-field-bg);color:var(--nbf-txt);font-size:12px;"><button id="nbf-champ-create-event-btn" ' + (!seasons.length ? 'disabled ' : '') + 'style="padding:7px 12px;background:' + (!seasons.length ? '#94a3b8' : '#16a34a') + ';color:#fff;border:none;border-radius:6px;font-size:12px;font-weight:600;cursor:' + (!seasons.length ? 'not-allowed' : 'pointer') + ';">Create Round/Event</button></div>' +
      '</div>' +
      '<div style="border:1px solid var(--nbf-bld);border-radius:7px;padding:10px;background:var(--nbf-main);margin-bottom:12px;"><h4 style="margin:0 0 4px;color:var(--nbf-txt);font-size:12px;">3. Assign Race to Round/Event</h4><div style="font-size:11px;color:var(--nbf-mut);margin-bottom:8px;">Only assigned races count toward selected series standings.</div><div style="display:grid;grid-template-columns:1fr 1fr auto;gap:8px;"><select id="nbf-champ-assign-race" style="width:100%;box-sizing:border-box;padding:7px 9px;border:1px solid var(--nbf-btn-border);border-radius:6px;background:var(--nbf-field-bg);color:var(--nbf-txt);font-size:12px;">' + raceOptions + '</select><select id="nbf-champ-assign-event" style="width:100%;box-sizing:border-box;padding:7px 9px;border:1px solid var(--nbf-btn-border);border-radius:6px;background:var(--nbf-field-bg);color:var(--nbf-txt);font-size:12px;">' + eventOptions + '</select><button id="nbf-champ-assign-btn" ' + (!events.length || !unassigned.length ? 'disabled ' : '') + 'style="padding:7px 12px;background:' + (!events.length || !unassigned.length ? '#94a3b8' : '#6366f1') + ';color:#fff;border:none;border-radius:6px;font-size:12px;font-weight:600;cursor:' + (!events.length || !unassigned.length ? 'not-allowed' : 'pointer') + ';">Assign</button></div></div>' +
      '<h4 style="margin:0 0 8px;color:var(--nbf-txt);font-size:12px;">Championship Standings</h4>';

    if (!standings.length) {
      html += '<div style="font-size:12px;color:var(--nbf-mut);padding:8px 0;">No assigned race results yet.</div>';
    } else {
      html += '<div style="overflow:auto;border:1px solid var(--nbf-bld);border-radius:7px;"><table style="width:100%;border-collapse:collapse;font-size:11px;"><thead><tr style="background:var(--nbf-alt);color:var(--nbf-txt);"><th style="text-align:left;padding:6px;">#</th><th style="text-align:left;padding:6px;">Driver</th><th style="text-align:right;padding:6px;">Points</th><th style="text-align:right;padding:6px;">Wins</th><th style="text-align:right;padding:6px;">Races</th></tr></thead><tbody>';
      standings.forEach(function(r, i) {
        html += '<tr style="border-top:1px solid var(--nbf-bld);"><td style="padding:6px;">' + (i + 1) + '</td><td style="padding:6px;">' + escapeHTML(r.driver_name || r.name || 'Unknown') + '</td><td style="padding:6px;text-align:right;font-weight:700;">' + escapeHTML(r.points || 0) + '</td><td style="padding:6px;text-align:right;">' + escapeHTML(r.wins || 0) + '</td><td style="padding:6px;text-align:right;">' + escapeHTML(r.races || 0) + '</td></tr>';
      });
  ...
  function bindChampionshipButtons() {
    function bindClick(id, handler, key) {
      var el = document.getElementById(id);
      if (!el || typeof handler !== 'function') return;

      var guardKey = 'nbfBound' + key;
      if (el.dataset && el.dataset[guardKey] === '1') return;
      if (el.dataset) el.dataset[guardKey] = '1';

      el.addEventListener('click', function(e) {
        e.preventDefault();
        e.stopPropagation();
        console.log('[NB Racing] Championship button clicked:', id);
        handler();
      });
    }

    bindClick('nbf-champ-refresh-btn', fetchChampionshipData, 'Refresh');
    bindClick('nbf-champ-create-season-btn', createChampionshipSeason, 'CreateSeason');
    bindClick('nbf-champ-create-event-btn', createChampionshipEvent, 'CreateEvent');
    bindClick('nbf-champ-assign-btn', assignRaceToChampionshipEvent, 'AssignRace');
  
    var viewStandingsBtn = document.getElementById('nbf-champ-view-standings');
    if (viewStandingsBtn) nbfBindOnce(viewStandingsBtn, 'click', function() {
      var seasonSelect = document.getElementById('nbf-champ-event-season') || document.getElementById('nbf-champ-season-select');
      var seasonId = seasonSelect && seasonSelect.value ? seasonSelect.value : '';
      if (seasonId) setStandingsScopeAndOpen('season:' + seasonId);
      else setStandingsScopeAndOpen('all');
    }, 'champViewStandings');
  ...
  function renderStewardPanelContent() {
    ...
    if (isAdminUnlocked() && !COMPETITION_SETUP_STATE.loading && !COMPETITION_SETUP_STATE.templates && !COMPETITION_SETUP_STATE.message) window.setTimeout(fetchCompetitionSetupData, 80);
    var targetContainer = document.getElementById('nbf-layout-body');
    if (!targetContainer) return;
    ...
    var adminHtml = '' +
      ...
      renderControlAccordion('admin-competition-manager', 'Series Manager', (COMPETITION_SETUP_STATE.competitions.length || 0) + ' competition(s)', renderCompetitionSetupBody, false) +
    ...
    var stewardHtml = '' +
      ...
      renderControlAccordion('steward-upload-status', 'Upload Status', (LAST_INGESTION_STATUS.message || 'No upload yet').slice(0, 120), renderStewardUploadStatusBody, false) +
      renderControlAccordion('steward-upload-queue-forced', 'Upload Queue', (loadUploadQueue().length || 0) + ' item(s)', renderSafeUploadQueueBody, false) +
    ...
    var systemHealthBtn = document.getElementById('nbf-system-health-run-btn');
    if (systemHealthBtn) systemHealthBtn.addEventListener('click', function() {
      refreshBackendStatus(true);
      if (typeof runDatabaseToolsDiagnostics === 'function') runDatabaseToolsDiagnostics();
      if (typeof window.NBF_UPDATE_LAUNCHER_STATUS === 'function') window.NBF_UPDATE_LAUNCHER_STATUS();
    });

    var systemHealthDbBtn = document.getElementById('nbf-system-health-database-btn');
    if (systemHealthDbBtn) systemHealthDbBtn.addEventListener('click', function() {
      CONTROL_ACCORDION_STATE.adminDatabaseTools = true;
      renderStewardPanelContent();
    });

    var archiveCheckBtn = document.getElementById('nbf-archive-check-btn');
    if (archiveCheckBtn) archiveCheckBtn.addEventListener('click', function() {
      adminBackendRequest(CF_ADMIN_ARCHIVE_ENDPOINT, { method: 'GET' }, 6000)
        .then(function(res) {
          if (res.status === 404) throw new Error('Archive endpoint not installed yet.');
          if (!res.ok) throw new Error('Archive endpoint HTTP ' + res.status);
          showScraperToast('Archive endpoint responded.', 'success', 2500);
        })
        .catch(function(err) {
          showScraperToast(err && err.message ? err.message : String(err), 'warning', 4500);
        });
    });
    ...
    if (saveBtn) saveBtn.addEventListener('click', function() {
      ...
      secureSetValue(CONFIG_STOKEN_KEY, val);
      try { localStorage.removeItem(CONFIG_STOKEN_KEY); } catch (err) {}
  ...
  function transmitRacePayload(payload) {
    console.log('[NB Racing] transmitRacePayload called with:', payload);
    ...
    if (!payload || !payload.race_id || !Array.isArray(payload.results) || !payload.results.length) {
      console.error('[NB Racing] Invalid payload:', payload);
  ...
  function getSyncProgressState() {
    try {
      if (!window.__NBF_SYNC_PROGRESS_STATE) {
        window.__NBF_SYNC_PROGRESS_STATE = {
          active: false,
          factionIndex: 0,
          factionTotal: 0,
          driverIndex: 0,
          ...
          message: '',
          lastRenderAt: 0,
          lastRenderedDriverIndex: -1,
          lastRenderedFactionIndex: -1
        };
      }
      return window.__NBF_SYNC_PROGRESS_STATE;
    } catch (err) {
      return {
        active: false,
        factionIndex: 0,
  ...
  function safeUpdateLeaderboardSyncProgress(patch, forceRender) {
    try {
      var state = getSyncProgressState();
      Object.assign(state, patch || {});

      var now = Date.now();
      var driverIndex = Number(state.driverIndex || 0);
      var factionIndex = Number(state.factionIndex || 0);

      var shouldRender = !!forceRender ||
        !state.lastRenderAt ||
        now - state.lastRenderAt > 1000 ||
        driverIndex === 0 ||
        driverIndex % 10 === 0 ||
        factionIndex !== state.lastRenderedFactionIndex ||
        !state.active;

      if (!shouldRender) return;

      state.lastRenderAt = now;
      state.lastRenderedDriverIndex = driverIndex;
      state.lastRenderedFactionIndex = factionIndex;
      safeRenderLeaderboardSyncProgress();
    } catch (err) {
      // Never let progress UI interfere with the API sync loop.
    }
  }

  function safeRenderLeaderboardSyncProgress() {
    try {
      var state = getSyncProgressState();
      var node = document.getElementById('nbf-layout-progress');
      if (!node) return;
      ...
      var factionTotal = Math.max(1, Number(state.factionTotal || TARGET_FACTIONS.length || 1));
      var factionIndex = Math.max(0, Number(state.factionIndex || 0));
      var driverTotal = Math.max(1, Number(state.driverTotal || 1));
      var driverIndex = Math.max(0, Number(state.driverIndex || 0));

      var pct = Math.round(Math.max(0, Math.min(100,
        ((Math.min(factionIndex, factionTotal) / factionTotal) +
        ((Math.min(driverIndex, driverTotal) / driverTotal) / factionTotal)) * 100
      )));

      var msg = state.message || 'Syncing leaderboard...';
      var factionLine = state.currentFaction
        ? 'Faction ' + Math.min(factionIndex + 1, factionTotal) + ' / ' + factionTotal + ' · ' + state.currentFaction
        : 'Faction ' + Math.min(factionIndex + 1, factionTotal) + ' / ' + factionTotal;

      var driverLine = state.currentDriver
        ? 'Driver ' + Math.min(driverIndex + 1, driverTotal) + ' / ' + driverTotal + ' · ' + state.currentDriver
        : (driverTotal > 1 ? 'Driver ' + Math.min(driverIndex + 1, driverTotal) + ' / ' + driverTotal : 'Preparing drivers');

      node.style.display = 'block';
      node.innerHTML =
        '<div style="padding:7px 12px; background:#eff6ff; border-bottom:1px solid #bfdbfe; color:#1e40af;">' +
        '  <div style="display:flex; justify-content:space-between; align-items:center; gap:10px; font-size:11px; font-weight:800; margin-bottom:5px;">' +
        '    <span>' + escapeHTML(msg) + '</span>' +
        '    <span>' + pct + '%</span>' +
        '  </div>' +
        '  <div style="height:7px; border-radius:999px; background:#dbeafe; overflow:hidden;">' +
        '    <div style="height:100%; width:' + pct + '%; border-radius:999px; background:#2563eb; transition:width .2s ease;"></div>' +
        '  </div>' +
        '  <div style="display:flex; justify-content:space-between; gap:10px; margin-top:5px; font-size:10px; color:#1d4ed8; font-weight:700;">' +
        '    <span>' + escapeHTML(factionLine) + '</span>' +
        '    <span>' + escapeHTML(driverLine) + '</span>' +
        '  </div>' +
        '</div>';
    } catch (err) {
      // Progress UI is non-critical.
    }
  }
...
function baseCacheRouter(bypassCacheMode) {
    ...
    if (!bypassCacheMode && targetCacheBlob && timestampMarker) {
      ...
      if (ageOfData < CACHE_DURATION) {
        ...
        RUNTIME_MAX_SKILL = skillsArray.length ? Math.max.apply(Math, skillsArray) : 1;
    if (RUNTIME_MAX_SKILL < 1) RUNTIME_MAX_SKILL = 1;
        ...
        safeUpdateLeaderboardSyncProgress({ active: false, message: 'Cache active.' }, true);
        routerViewRefresh();
        return;
      }
    }
    ...
    RUNTIME_MEMBERS  = [];
    safeUpdateLeaderboardSyncProgress({ active: true, factionIndex: 0, factionTotal: TARGET_FACTIONS.length, driverIndex: 0, driverTotal: 0, currentFaction: '', currentDriver: '', message: 'Starting leaderboard sync...' }, true);
  ...
  function processFactionQueueBatches(factionPointer) {
    ...
    safeUpdateLeaderboardSyncProgress({
      active: true,
      factionIndex: factionPointer,
      factionTotal: TARGET_FACTIONS.length,
      driverIndex: 0,
      driverTotal: 0,
      currentFaction: activeTargetFactionObj.name,
      currentDriver: '',
      message: 'Loading faction roster...'
    }, true);
    ...
    fetch('https://api.torn.com/faction/' + activeTargetFactionObj.id + '?selections=basic&key=' + STATE_API_KEY)
      ...
      .then(function(payload) {
        ...
        safeUpdateLeaderboardSyncProgress({ active: true, factionIndex: factionPointer, factionTotal: TARGET_FACTIONS.length, driverIndex: 0, driverTotal: extractedProfileCollection.length, currentFaction: activeTargetFactionObj.name, currentDriver: '', message: 'Syncing driver stats...' }, true);
  ...
  function processIndividualDriversSequence(profileCollection, memberIndex, continuousCallback) {
    ...
    safeUpdateLeaderboardSyncProgress({
      active: true,
      driverIndex: memberIndex,
      driverTotal: profileCollection.length,
      currentDriver: trackingMemberNode.name,
      message: 'Syncing leaderboard driver stats...'
    }, false);
  ...
  function completePipelineProcessing() {
    safeUpdateLeaderboardSyncProgress({
      active: true,
      factionIndex: TARGET_FACTIONS.length,
      factionTotal: TARGET_FACTIONS.length,
      driverIndex: 1,
      driverTotal: 1,
      currentFaction: '',
      currentDriver: '',
      message: 'Finalizing leaderboard cache...'
    }, true);

    var calculatedSkillsArray = RUNTIME_MEMBERS.map(function(m) { return m.racing_skill || 0; });
    RUNTIME_MAX_SKILL = calculatedSkillsArray.length ? Math.max.apply(Math, calculatedSkillsArray) : 1;
    if (RUNTIME_MAX_SKILL < 1) RUNTIME_MAX_SKILL = 1;
    ...
    window.setTimeout(function() {
      safeUpdateLeaderboardSyncProgress({ active: false, message: '' }, true);
    }, 900);
    routerViewRefresh();
  }
  ...
  function nbfDocBox(type, title, body) {
    var map = { info:['#dbeafe','#1d4ed8','ℹ️'], tip:['#dcfce7','#15803d','💡'], warn:['#fef3c7','#b45309','⚠️'], soon:['#f3e8ff','#7e22ce','🚧'], secure:['#e0f2fe','#0369a1','🔒'] };
    var c = map[type] || map.info;
    return '<div style="border:1px solid '+c[1]+'33;background:'+c[0]+';border-radius:10px;padding:12px 14px;margin:12px 0;"><div style="font-weight:800;color:'+c[1]+';font-size:13px;margin-bottom:5px;">'+c[2]+' '+escapeHTML(title)+'</div><div style="font-size:12px;line-height:1.55;color:#334155;">'+body+'</div></div>';
  }

  function nbfDocStep(num, title, body, tip) {
    return '<div style="display:grid;grid-template-columns:42px 1fr;gap:12px;margin:12px 0;align-items:flex-start;"><div title="'+escapeHTML(tip||title)+'" style="width:34px;height:34px;border-radius:50%;background:#6366f1;color:#fff;display:flex;align-items:center;justify-content:center;font-weight:900;box-shadow:0 4px 12px rgba(99,102,241,.25);cursor:help;">'+escapeHTML(num)+'</div><div style="border:1px solid var(--nbf-bld);background:var(--nbf-main);border-radius:10px;padding:10px 12px;"><div style="font-size:13px;font-weight:800;color:var(--nbf-txt);margin-bottom:4px;">'+escapeHTML(title)+'</div><div style="font-size:12px;line-height:1.55;color:var(--nbf-mut);">'+body+'</div></div></div>';
  }

  function nbfDocProgress(label, pct) {
    var blocks = Math.max(0, Math.min(10, Math.round(Number(pct || 0) / 10)));
    return '<div style="display:grid;grid-template-columns:190px 110px 48px;gap:8px;align-items:center;font-size:12px;margin:4px 0;"><div style="color:var(--nbf-txt);font-weight:700;">'+escapeHTML(label)+'</div><div style="font-family:monospace;letter-spacing:.5px;color:#6366f1;">'+'█'.repeat(blocks)+'░'.repeat(10-blocks)+'</div><div style="color:var(--nbf-mut);text-align:right;">'+escapeHTML(pct)+'%</div></div>';
  }

  function nbfDocFlow(items) {
    return '<div style="display:flex;flex-wrap:wrap;gap:8px;align-items:center;margin:12px 0;">'+items.map(function(item,i){ var text=(item.text||item); return '<span title="'+escapeHTML(item.tip||text)+'" style="display:inline-flex;align-items:center;gap:6px;"><span style="background:var(--nbf-alt);border:1px solid var(--nbf-bld);border-radius:999px;padding:6px 10px;font-size:11px;font-weight:800;color:var(--nbf-txt);cursor:help;">'+escapeHTML(text)+'</span>'+(i<items.length-1?'<span style="color:var(--nbf-mut);font-weight:900;">→</span>':'')+'</span>'; }).join('')+'</div>';
  }

  function nbfDocTable(rows) {
    return '<div style="overflow:auto;border:1px solid var(--nbf-bld);border-radius:10px;margin:10px 0;"><table style="width:100%;border-collapse:collapse;font-size:12px;">'+rows.map(function(row,idx){ return '<tr>'+row.map(function(cell){ return (idx===0?'<th':'<td')+' style="padding:8px 10px;border-bottom:1px solid var(--nbf-bld);text-align:left;color:var(--nbf-'+(idx===0?'txt':'mut')+');background:'+(idx===0?'var(--nbf-alt)':'transparent')+';">'+cell+(idx===0?'</th>':'</td>'); }).join('')+'</tr>'; }).join('')+'</table></div>';
  }

  function nbfArticleWelcome() {
    return '<h2 style="margin:0 0 8px;color:var(--nbf-txt);">Nuclear Family Championship Engine</h2><p style="font-size:13px;line-height:1.6;color:var(--nbf-mut);">The Nuclear Family Engine is a Torn Racing competition platform. It combines a browser userscript, Cloudflare Worker backend, D1 database, steward authentication, verified race uploads, participant imports, and reusable competition templates.</p>'+
      nbfDocBox('info','What this is for','The goal is to remove repetitive spreadsheet work from Torn racing competitions. Organizers configure a competition, import participants, collect verified race uploads, and let the engine produce structured standings and participant records.')+
      nbfDocFlow([{text:'Torn Race Log',tip:'A completed Torn race page is detected.'},{text:'Upload Queue',tip:'Uploads are queued and retried if needed.'},{text:'Cloudflare Worker',tip:'Backend validates and stores data.'},{text:'D1 Database',tip:'Central store for races, participants and audit records.'},{text:'Competition Engine',tip:'Templates transform verified data into standings.'}])+
      nbfDocBox('tip','Organizer pitch','For JLT or URT organizers, this does not force a new format. It provides templates designed around those workflows so the engine supports existing formats rather than replacing them.');
  }

  function nbfArticleGettingStarted() {
    return '<h2 style="margin:0 0 8px;color:var(--nbf-txt);">Getting Started</h2>'+
      nbfDocStep(1,'Install the userscript','Install in Violentmonkey or Tampermonkey. Open Torn Racing and use the floating button to launch the dashboard.','Start here.')+
      nbfDocStep(2,'Add your Torn API key','Used for local racing profile data and dashboard enrichment. It is stored in userscript storage, not in the league database.','Your Torn API key is not a steward token.')+
      nbfDocStep(3,'Login as steward','Open Control Center and login with your Torn ID and personal steward token. The backend stores only a hash.','Use nf_steward_..., not nf_session_...')+
      nbfDocStep(4,'Open completed race logs','The focused race log is checked, extracted, queued, uploaded and verified.','Background race pages are skipped intentionally.')+
      nbfDocStep(5,'Create a competition template','In Series Manager, choose JLT, URT or Custom. Templates store rules for grouping, scoring and advancement.','Templates are rule blueprints.')+
      nbfDocStep(6,'Import participants','Paste CSV, TSV, Excel copy/paste, or Google Sheets copy/paste. Torn ID is the strongest identity.','Names can change; Torn IDs are stable.');
  }

  function nbfArticleUploads() {
    return '<h2 style="margin:0 0 8px;color:var(--nbf-txt);">Race Upload System</h2><p style="font-size:13px;line-height:1.6;color:var(--nbf-mut);">The upload system waits for a focused completed race log, extracts the leaderboard, fingerprints the payload, queues the upload, and verifies storage with the backend.</p>'+
      nbfDocFlow(['Race detected','Focus check','Payload built','First confirmation','Queue','POST /ingest-race','Verify','Complete'])+
      nbfDocBox('secure','Why focus matters','The script only scrapes race logs while the Torn tab is focused. This avoids background scraping behavior and keeps upload actions intentional.')+
      nbfDocTable([['Status','Meaning','Organizer action'],['Queued','Race payload is waiting locally.','Usually none.'],['Uploading','Payload is being sent to Cloudflare.','Wait.'],['Verifying','Backend accepted the race and storage is checked.','Wait.'],['Completed','Race is stored and verified.','No action.'],['Retrying','A network/backend problem happened.','Let retry or inspect status.'],['Duplicate','Race already exists.','Usually no action.']]);
  }

  function nbfArticleStewards() {
    return '<h2 style="margin:0 0 8px;color:var(--nbf-txt);">Steward System</h2><p style="font-size:13px;line-height:1.6;color:var(--nbf-mut);">Stewards are backend identities. The UI can display buttons, but the Cloudflare Worker decides whether a request is allowed.</p>'+
      nbfDocTable([['Role','Typical use','Permissions'],['Super Admin','League owner','Everything: stewards, tokens, competitions, uploads.'],['Admin','Competition administrator','Manage competitions and stewards where allowed.'],['Chief','Senior race steward','Manage competition setup and review workflows.'],['Event','Race steward','Upload valid race results.'],['Observer','Read-only/helper','View-only tools where supported.']])+
      nbfDocBox('secure','Token model','Personal steward tokens are shown once. The backend stores only a hash. Login creates a session token stored for the browser session only.')+
      nbfDocBox('warn','Do not share tokens','If a steward token is exposed, regenerate it immediately in the Steward Registry.');
  }

  function nbfArticleTemplates() {
    return '<h2 style="margin:0 0 8px;color:var(--nbf-txt);">Series Templates</h2><p style="font-size:13px;line-height:1.6;color:var(--nbf-mut);">A template is a rule blueprint. It defines registration, grouping, scheduling, scoring and advancement.</p>'+
      nbfDocFlow(['Competition','Template','Participants','Divisions / Groups','Events / Races','Standings'])+
      nbfDocTable([['Template','Best for','Core logic'],['JLT-style League','League/group championships','Registered racers are grouped by class/RS and scored by group-size descending points.'],['URT-style Qualifier','Time-trial qualifiers','Fastest valid times on selected tracks qualify drivers for a tournament/final phase.'],['Custom','Experimental competitions','Generic setup for future formats.']])+
      nbfDocBox('tip','Why templates matter','Hardcoding JLT or URT would make future formats difficult. A template-driven setup lets new competition formats become configuration instead of rewriting the upload pipeline.');
  }

  function nbfArticleJLT() {
    return '<h2 style="margin:0 0 8px;color:var(--nbf-txt);">JLT-style League Template</h2><p style="font-size:13px;line-height:1.6;color:var(--nbf-mut);">The JLT-style template is designed for league/group based championships. It supports registration, class and Racing Skill grouping, a qualification phase, championship week and group-size based points.</p>'+
      nbfDocBox('info','Template identity','JLT is treated as a league championship, not a fastest-time qualifier. Participants are sorted into leagues/groups, race in scheduled phases, and score against their registered group size.')+
      '<h3 style="color:var(--nbf-txt);margin-top:18px;">Organizer workflow</h3>'+nbfDocFlow(['Create JLT competition','Import roster','Generate leagues','Run QP races','Collect uploads','Score groups','Run Championship Week','Publish champion'])+
      '<h3 style="color:var(--nbf-txt);margin-top:18px;">Registration</h3><p style="font-size:12px;line-height:1.6;color:var(--nbf-mut);">Registration starts with a participant database. The importer accepts pasted CSV/TSV rows, Excel copy/paste and Google Sheets copy/paste. Recommended columns are torn_id, display_name, faction, racing_skill, racer_class and car_class.</p>'+
      nbfDocBox('tip','Roster imports','If an organizer already maintains a RaceDOC or spreadsheet, copy the relevant columns into Participant Import. Direct XLSX/Google Sheets sync can be added later.')+
      '<h3 style="color:var(--nbf-txt);margin-top:18px;">League assignment</h3><p style="font-size:12px;line-height:1.6;color:var(--nbf-mut);">The template stores grouping rules: group by racer class, sort by Racing Skill, and split oversized leagues into groups. The default max group size is 80 racers.</p>'+
      nbfDocTable([['Rule','Default','Purpose'],['Group by class','Enabled','Keeps racers in appropriate class pools.'],['Sort by Racing Skill','Enabled','Supports fairer league bands.'],['Max group size','80','Splits very large leagues into multiple groups.'],['Participant status','registered','Tracks active/withdrawn/replacement racers later.']])+
      '<h3 style="color:var(--nbf-txt);margin-top:18px;">Qualification Phase</h3><p style="font-size:12px;line-height:1.6;color:var(--nbf-mut);">Default configuration: seven qualification races at fifty laps. These values are template config and can become organizer-editable.</p>'+
      '<h3 style="color:var(--nbf-txt);margin-top:18px;">Championship Week</h3><p style="font-size:12px;line-height:1.6;color:var(--nbf-mut);">Default configuration: seven championship races at twenty laps. Verified uploads can later count automatically toward the correct week, group and division.</p>'+
      '<h3 style="color:var(--nbf-txt);margin-top:18px;">Points model</h3>'+nbfDocBox('info','Group-size descending points','If a group has 50 registered racers, 1st receives 50 points, 2nd receives 49, and so on. Points are based on registered group size, not only the number who show up.')+
      nbfDocTable([['Example group size','Finish','Points'],['50 registered','1st','50'],['50 registered','2nd','49'],['50 registered','10th','41'],['50 registered','Missed race','0']])+
      '<h3 style="color:var(--nbf-txt);margin-top:18px;">Current status</h3>'+nbfDocProgress('Participant Database',100)+nbfDocProgress('Race Uploads',100)+nbfDocProgress('Verification',100)+nbfDocProgress('Template Storage',100)+nbfDocProgress('League Generator',40)+nbfDocProgress('Schedule Generator',30)+nbfDocProgress('Automatic JLT Standings',30)+
      nbfDocBox('soon','Planned next steps','Automatic league generation, editable template config, race-to-event helpers, JLT-specific standings and tie-breaker configuration.');
  }

  function nbfArticleURT() {
    return '<h2 style="margin:0 0 8px;color:var(--nbf-txt);">URT-style Qualifier Template</h2><p style="font-size:13px;line-height:1.6;color:var(--nbf-mut);">The URT-style template is designed for qualification tournaments. Instead of league points, it focuses on fastest valid times on selected tracks and advancement into a main tournament or final phase.</p>'+
      nbfDocBox('info','Template identity','URT is treated as a qualifier format. The engine identifies participants, collects verified uploads, ranks times per track, and selects top qualifiers according to configured advancement rules.')+
      '<h3 style="color:var(--nbf-txt);margin-top:18px;">Organizer workflow</h3>'+nbfDocFlow(['Create URT competition','Import roster','Configure tracks','Collect qualifier uploads','Rank fastest times','Select finalists','Run tournament','Publish champion'])+
      '<h3 style="color:var(--nbf-txt);margin-top:18px;">Qualifier structure</h3><p style="font-size:12px;line-height:1.6;color:var(--nbf-mut);">The default URT-style configuration stores multiple qualifier tracks, default lap count, and fastest_per_track. A common setup is fastest 16 per track, but it remains configurable.</p>'+
      nbfDocTable([['Rule','Default','Purpose'],['Qualifier tracks','Organizer-defined','Each track has its own ranking.'],['Lap count','10','Short qualifiers reduce time burden and increase accessibility.'],['Fastest per track','16','Top N advance from each track.'],['Best time per player','Enabled','A player is ranked by best valid attempt per track.']])+
      '<h3 style="color:var(--nbf-txt);margin-top:18px;">Fastest-time ranking</h3>'+nbfDocBox('tip','Different from JLT','URT does not need group-size descending points for qualifiers. The core result is time: fastest valid time per track wins the ranking slot.')+
      nbfDocFlow(['Track selected','Race uploaded','Time parsed','Best time stored','Ranking updated','Top N qualify'])+
      '<h3 style="color:var(--nbf-txt);margin-top:18px;">Current status</h3>'+nbfDocProgress('Participant Database',100)+nbfDocProgress('Race Uploads',100)+nbfDocProgress('Verification',100)+nbfDocProgress('Template Storage',100)+nbfDocProgress('Qualifier Track Config',35)+nbfDocProgress('Fastest-Time Ranking',25)+nbfDocProgress('Finalist Generation',15)+nbfDocProgress('Bracket Tools',10)+
      nbfDocBox('soon','Planned next steps','Editable track configuration, fastest-time standings, top-N finalist generation and bracket/final phase tools.');
  }

  function nbfArticleImport() {
    return '<h2 style="margin:0 0 8px;color:var(--nbf-txt);">Participant Import</h2><p style="font-size:13px;line-height:1.6;color:var(--nbf-mut);">Participant import bridges existing organizer spreadsheets and the backend participant database.</p>'+
      nbfDocBox('tip','Supported today','Paste CSV, TSV, Excel copy/paste or Google Sheets copy/paste. Direct XLSX upload and Google Sheets API sync are future enhancements.')+
      '<pre style="background:var(--nbf-alt);border:1px solid var(--nbf-bld);border-radius:10px;padding:12px;font-size:11px;overflow:auto;color:var(--nbf-mut);">torn_id,display_name,faction,racing_skill,racer_class,car_class\n1496324,cowboyup,Nuclear Family,92.4,A,A\n1234567,Example Racer,Other Faction,41.8,B,B</pre>'+
      nbfDocTable([['Column','Required','Notes'],['torn_id','Recommended','Best stable identifier.'],['display_name','Recommended','Used in UI and fallback matching.'],['faction','Optional','Useful for faction/team stats.'],['racing_skill','Optional','Needed for JLT-style RS grouping.'],['racer_class','Optional','Needed for class-based grouping.'],['car_class','Optional','Useful for restrictions or reporting.'],['status','Optional','Defaults to registered.'],['notes','Optional','Organizer notes.']])+
      nbfDocBox('warn','Data quality matters','If Torn IDs are missing, matching falls back to names. That works for testing but is weaker because names can change.');
  }

  function nbfArticleSecurity() {
    return '<h2 style="margin:0 0 8px;color:var(--nbf-txt);">Security and Audit Model</h2><p style="font-size:13px;line-height:1.6;color:var(--nbf-mut);">The engine separates local UI state from backend authority. The browser can display buttons, but the Worker decides whether a request is allowed.</p>'+
      nbfDocBox('secure','No Torn passwords','The engine never asks for a Torn password. It uses Torn API keys for local profile features and personal steward tokens for league authority.')+
      nbfDocTable([['Layer','Protection'],['Torn API key','Stored in userscript storage; used for local features.'],['Steward token','Shown once; backend stores only a hash.'],['Session token','Created after login; stored only for browser session.'],['Race uploads','Require authorized steward/session and are duplicate protected.'],['Audit log','Backend records important actions like login, imports and race ingestion.']])+
      nbfDocBox('warn','Admin token is emergency mode','Cloudflare ADMIN_TOKEN should be treated as bootstrap/recovery. Normal operations should use personal steward login.');
  }

  
  function roadmapRow(module, status, progress, next) {
    return '<tr>' +
      '<td style="padding:7px 8px; border:1px solid var(--nbf-bld); color:var(--nbf-txt); font-weight:700;">' + escapeHTML(module) + '</td>' +
      '<td style="padding:7px 8px; border:1px solid var(--nbf-bld); text-align:center;">' + escapeHTML(status) + '</td>' +
      '<td style="padding:7px 8px; border:1px solid var(--nbf-bld); text-align:center; color:var(--nbf-txt); font-weight:700;">' + escapeHTML(progress) + '</td>' +
      '<td style="padding:7px 8px; border:1px solid var(--nbf-bld); color:var(--nbf-mut);">' + escapeHTML(next) + '</td>' +
      '</tr>';
  }
  ...
  function priorityTable(items) {
    return '<table style="width:100%; border-collapse:collapse; font-size:12px;">' +
      items.map(function(item) {
        return '<tr>' +
          '<td style="width:42px; padding:6px 8px; border:1px solid var(--nbf-bld); text-align:center; color:#f59e0b; font-weight:800;">TODO</td>' +
          '<td style="padding:6px 8px; border:1px solid var(--nbf-bld); color:var(--nbf-txt);">' + escapeHTML(item) + '</td>' +
          '</tr>';
      }).join('') +
      '</table>';
  }

function nbfArticleRoadmap() {
    return '' +
      '<h2 style="margin:0 0 10px;color:var(--nbf-txt);">Roadmap</h2>' +
      '<p style="margin:0 0 12px;font-size:12px;color:var(--nbf-mut);">Data-focused project status. No filler.</p>' +

      '<table style="width:100%; border-collapse:collapse; font-size:12px; margin-bottom:14px;">' +
      '  <thead><tr style="background:var(--nbf-alt2); color:var(--nbf-txt);">' +
      '    <th style="text-align:left; padding:8px; border:1px solid var(--nbf-bld);">Module</th>' +
      '    <th style="text-align:center; padding:8px; border:1px solid var(--nbf-bld);">Status</th>' +
      '    <th style="text-align:center; padding:8px; border:1px solid var(--nbf-bld);">Progress</th>' +
      '    <th style="text-align:left; padding:8px; border:1px solid var(--nbf-bld);">Next</th>' +
      '  </tr></thead>' +
      '  <tbody>' +
      roadmapRow('Leaderboard', '✅', '100%', '—') +
      roadmapRow('Upload Queue', '✅', '100%', '—') +
      roadmapRow('League Setup', '✅', '100%', 'Standings selector verification') +
      roadmapRow('Standings', '🟡', '80%', 'Backend filter support / live test') +
      roadmapRow('Database Tools', '🟡', '90%', 'Deploy Worker v1.0.13 + test reset') +
      roadmapRow('Archive', '🟡', '50%', 'Verify /admin/archive after Worker deploy') +
      roadmapRow('Backend', '🟡', '90%', 'Deploy v1.0.13 admin maintenance') +
      roadmapRow('Help Central', '✅', '100%', 'Keep data focused') +
      roadmapRow('Security', '🟢', '95%', 'Final audit') +
      roadmapRow('Release Candidate', '🔴', '70%', 'Finish P0 backend verification') +
      '  </tbody>' +
      '</table>' +

      '<h3 style="margin:12px 0 8px;color:var(--nbf-txt);">P0 — Next</h3>' +
      priorityTable(['Deploy Worker v1.0.13 admin maintenance backend', 'Verify /admin/dev-reset, /admin/archive, /admin/health, /admin/config-check', 'Test factory reset on test data only']) +

      '<h3 style="margin:12px 0 8px;color:var(--nbf-txt);">P1 — After P0</h3>' +
      priorityTable(['Standings by Series live backend verification', 'Archive workflow UI activation', 'End-to-end release test']) +

      '<h3 style="margin:12px 0 8px;color:var(--nbf-txt);">P2 — Release Prep</h3>' +
      priorityTable(['GreasyFork screenshots and description', 'Backend deployment checklist', 'Admin/steward quick-start docs']) +

      '<div style="margin-top:12px; padding:10px; border:1px solid var(--nbf-bld); border-radius:8px; background:var(--nbf-alt2); font-size:12px; color:var(--nbf-mut);">' +
      '<strong style="color:var(--nbf-txt);">Overall:</strong> 82% · Blocker: Worker v1.0.13 deployment and live backend verification.' +
      '</div>';
  }

  function renderHelpCentralDocs() {
    var docs = [
      ['welcome','Welcome',nbfArticleWelcome],
      ['getting','Getting Started',nbfArticleGettingStarted],
      ['uploads','Race Upload System',nbfArticleUploads],
      ['stewards','Steward System',nbfArticleStewards],
      ['templates','Series Templates',nbfArticleTemplates],
      ['jlt','JLT Setup',nbfArticleJLT],
      ['urt','URT Setup',nbfArticleURT],
      ['import','Participant Import',nbfArticleImport],
      ['security','Security',nbfArticleSecurity],
      ['roadmap','Roadmap',nbfArticleRoadmap]
    ];
    var active = sessionStorage.getItem('nbf_help_doc_active') || 'welcome';
    var selected = docs.find(function(d){ return d[0] === active; }) || docs[0];
    var nav = docs.map(function(d){
      var isActive = d[0] === selected[0];
      return '<button class="nbf-help-doc-nav" data-doc="'+escapeHTML(d[0])+'" title="Open '+escapeHTML(d[1])+' help" style="width:100%;text-align:left;padding:8px 10px;margin-bottom:5px;border:1px solid '+(isActive?'#6366f1':'var(--nbf-bld)')+';background:'+(isActive?'#eef2ff':'var(--nbf-main)')+';color:'+(isActive?'#3730a3':'var(--nbf-txt)')+';border-radius:8px;font-size:12px;font-weight:'+(isActive?'800':'600')+';cursor:pointer;">'+escapeHTML(d[1])+'</button>';
    }).join('');
    return '<div style="padding:20px;max-width:1180px;margin:0 auto;background:var(--nbf-main);font-family:sans-serif;"><div style="display:flex;justify-content:space-between;align-items:flex-start;gap:14px;border-bottom:1px solid var(--nbf-bld);padding-bottom:12px;margin-bottom:14px;"><div><h1 style="margin:0;font-size:18px;color:var(--nbf-txt);">Help Central</h1><p style="margin:5px 0 0;font-size:12px;color:var(--nbf-mut);">Built-in documentation for organizers, stewards, JLT-style leagues, URT-style qualifiers and future custom formats.</p></div><div title="Documentation version included in this userscript" style="font-size:11px;color:var(--nbf-mut);text-align:right;cursor:help;">Docs v1.0<br>Script v'+escapeHTML(ENGINE_VERSION)+'<br>Backend v1.0.12+</div></div><div style="display:grid;grid-template-columns:220px minmax(0,1fr);gap:16px;align-items:start;"><div style="position:sticky;top:8px;">'+nav+'</div><article style="border:1px solid var(--nbf-bld);border-radius:12px;padding:18px;background:var(--nbf-alt2);min-height:520px;">'+selected[2]()+'</article></div></div>';
  }

  function bindHelpCentralDocs() {
    var buttons = document.querySelectorAll('.nbf-help-doc-nav');
    Array.prototype.forEach.call(buttons, function(btn) {
      btn.addEventListener('click', function() {
        sessionStorage.setItem('nbf_help_doc_active', this.getAttribute('data-doc') || 'welcome');
        var target = document.getElementById('nbf-layout-body');
        if (target) {
          target.innerHTML = renderHelpCentralDocs();
          bindHelpCentralDocs();
        }
      });
    });
...
function renderHelpPanelContent() {
    var targetContainer = document.getElementById('nbf-layout-body');
    if (!targetContainer) return;
    targetContainer.innerHTML = renderHelpCentralDocs();
    bindHelpCentralDocs();
  }
  ...
  /**********************************************************************
   * BOOK 16 — SPA NAVIGATION & PAGE LIFECYCLE MANAGER
   *
   * Torn Racing submenus can swap DOM content without changing the visible
   * URL and without reloading userscripts. This manager keeps Nuclear Family
   * mounted across those internal page transitions.
   **********************************************************************/

  var NBF_LIFECYCLE = window.NBF_LIFECYCLE || {
    started: false,
    observer: null,
    timer: null,
    lastSignature: '',
  ...
  window.NBF_LIFECYCLE = NBF_LIFECYCLE;

  function nbfLifecycleOn(eventName, handler) {
    if (!eventName || typeof handler !== 'function') return;
    if (!NBF_LIFECYCLE.listeners[eventName]) NBF_LIFECYCLE.listeners[eventName] = [];
    NBF_LIFECYCLE.listeners[eventName].push(handler);
  }

  function nbfLifecycleEmit(eventName, payload) {
    var handlers = NBF_LIFECYCLE.listeners[eventName] || [];
    handlers.forEach(function(handler) {
      try { handler(payload || {}); }
      catch (err) { console.error('[NB Racing] Lifecycle handler failed:', eventName, err); }
    });
  }

  function nbfIsRacingPageNow() {
    try {
      var ctx = getRacingURLContext();
      if (ctx && ctx.isRacing) return true;
    } catch (err) {}

    try {
      return /sid=racing/i.test(location.search || location.href || '');
    } catch (err2) {
      return false;
    }
  }

  function nbfGetPageSignature() {
    var ctx = {};
    try { ctx = getRacingURLContext() || {}; } catch (err) {}

    var visibleTitle = '';
    try {
      var titleNode = document.querySelector('h1,h2,h3,h4,[class*="title" i],[class*="heading" i]');
      visibleTitle = titleNode ? String(titleNode.textContent || '').replace(/\s+/g, ' ').trim().slice(0, 140) : '';
    } catch (err2) {}
  ...
  function nbfEnsureRacingLauncher() {
    if (!nbfIsRacingPageNow()) return;

    try {
      var hasLauncher =
        document.getElementById('nbf-action-trigger-root') ||
        document.getElementById('nbf-action-trigger-fallback') ||
        document.getElementById('nbf-action-trigger-button');

      if (!hasLauncher) {
        mountFloatingInterface();
      } else if (typeof window.NBF_UPDATE_LAUNCHER_STATUS === 'function') {
        window.NBF_UPDATE_LAUNCHER_STATUS();
      }
    } catch (err) {
      console.error('[NB Racing] Launcher lifecycle refresh failed:', err);
    }
  }

  function nbfLifecycleRefresh(reason) {
    if (!nbfIsRacingPageNow()) return;

    var signature = nbfGetPageSignature();
    var changed = signature !== NBF_LIFECYCLE.lastSignature;
    NBF_LIFECYCLE.lastSignature = signature;

    nbfEnsureRacingLauncher();

    var context = {};
    try { context = getRacingURLContext() || {}; } catch (err) {}

    nbfLifecycleEmit('NBF_RACING_PAGE_READY', {
    ...
    if (context && context.tab === 'log' && context.raceID && typeof checkAndScrapeRace === 'function') {
      if (!document.hidden && document.hasFocus()) {
        try {
          var raceId = String(context.raceID || '');
          if (raceId && !SCRAPER_SESSION_RACES[raceId]) {
            SCRAPER_SESSION_RACES[raceId] = true;
            checkAndScrapeRace();
          }
        } catch (err2) {
          console.error('[NB Racing] Lifecycle race scraper check failed:', err2);
        }
      }
    }
  }

  function nbfScheduleLifecycleRefresh(reason) {
    window.clearTimeout(NBF_LIFECYCLE.timer);
    NBF_LIFECYCLE.timer = window.setTimeout(function() {
      nbfLifecycleRefresh(reason || 'scheduled');
    }, 140);
  }

  function nbfStartLifecycleManager() {
    if (NBF_LIFECYCLE.started) {
      nbfScheduleLifecycleRefresh('already-started');
      return;
    }

    NBF_LIFECYCLE.started = true;

    if (!history.__nbfLifecyclePatched) {
      var originalPushState = history.pushState;
      var originalReplaceState = history.replaceState;

      history.pushState = function() {
        var result = originalPushState.apply(this, arguments);
        nbfScheduleLifecycleRefresh('pushState');
        return result;
      };

      history.replaceState = function() {
        var result = originalReplaceState.apply(this, arguments);
        nbfScheduleLifecycleRefresh('replaceState');
        return result;
      };

      history.__nbfLifecyclePatched = true;
    }

    window.addEventListener('popstate', function() { nbfScheduleLifecycleRefresh('popstate'); });
    window.addEventListener('hashchange', function() { nbfScheduleLifecycleRefresh('hashchange'); });
    window.addEventListener('focus', function() { nbfScheduleLifecycleRefresh('focus'); });
    document.addEventListener('visibilitychange', function() {
      if (!document.hidden) nbfScheduleLifecycleRefresh('visibilitychange');
    });

    if (NBF_LIFECYCLE.observer) {
      try { NBF_LIFECYCLE.observer.disconnect(); } catch (err) {}
    }

    NBF_LIFECYCLE.observer = new MutationObserver(function(mutations) {
      if (!nbfIsRacingPageNow()) return;

      var hasLauncher = document.getElementById('nbf-action-trigger-button');
      var relevant = !hasLauncher;
      ...
      for (var i = 0; i < mutations.length && !relevant; i++) {
        ...
        for (var j = 0; j < mutation.addedNodes.length; j++) {
          ...
          if (node.id === 'nbf-action-trigger-root' || node.id === 'nbf-action-trigger-fallback' || node.id === 'nbf-backdrop-mask') continue;

          var txt = String(node.textContent || '').toLowerCase();
          if (
            txt.indexOf('race') !== -1 ||
            txt.indexOf('racing') !== -1 ||
            txt.indexOf('official events') !== -1 ||
            txt.indexOf('custom events') !== -1 ||
            txt.indexOf('your cars') !== -1 ||
            txt.indexOf('parts') !== -1 ||
            txt.indexOf('statistics') !== -1
          ) {
            relevant = true;
            break;
          }

          if (node.querySelector && node.querySelector('h1,h2,h3,h4,[class*="title" i],[class*="heading" i],[class*="racing" i]')) {
            relevant = true;
            break;
          }
        }
      ...
      if (relevant) nbfScheduleLifecycleRefresh('mutation');
    });

    if (document.body || document.documentElement) NBF_LIFECYCLE.observer.observe(document.body || document.documentElement, {
      childList: true,
      subtree: true
    });

    window.setInterval(function() {
      if (nbfIsRacingPageNow() && !document.getElementById('nbf-action-trigger-button')) {
        nbfScheduleLifecycleRefresh('safety-pulse');
      }
    }, 1500);

    nbfScheduleLifecycleRefresh('start');
  }
  ...
  if (racingContext.isRacing) {
    ...
    if (isLogPage) {
      window.addEventListener('focus', function() {
        bootRaceScraperIfNeeded();
        nbfScheduleLifecycleRefresh('race-log-focus');
      });
      document.addEventListener('visibilitychange', function() {
        bootRaceScraperIfNeeded();
        if (!document.hidden) nbfScheduleLifecycleRefresh('race-log-visibility');
      });
    }
  }
